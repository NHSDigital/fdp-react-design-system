...
var ve, Gr, Ve, Dt, Rt, nr, qn, Bt, mr, Kn, wt, ot, Zn, Jn = function() {
  ...
  var l = P[i + u], p = l || (P[i + u] = Zr(At(e, t), !0) || (wr(e) ? r : Zr(function(x) {
  ...
  return ve = e || Jn(), !Gr && ve && typeof document < "u" && document.body && (Ve = window, Dt = document, Rt = Dt.documentElement, nr = Dt.body, Kn = [Ve, Dt, Rt, nr], ve.utils.clamp, Zn = ve.core.context || function() {
...
var J = /* @__PURE__ */ (function() {
  ...
  return e.init = function(t) {
    ...
    var r = t.tolerance, i = t.dragMinimum, u = t.type, l = t.target, p = t.lineHeight, x = t.debounce, Y = t.preventDefault, M = t.onStop, h = t.onStopDelay, c = t.ignore, v = t.wheelSpeed, $ = t.event, X = t.onDragStart, ne = t.onDragEnd, W = t.onDrag, ge = t.onPress, k = t.onRelease, qe = t.onRight, N = t.onLeft, y = t.onUp, Te = t.onDown, Ie = t.onChangeX, g = t.onChangeY, ue = t.onChange, w = t.onToggleX, pt = t.onToggleY, ie = t.onHover, Ee = t.onHoverEnd, Pe = t.onMove, I = t.ignoreCheck, Q = t.isNormalizer, j = t.onGestureStart, s = t.onGestureEnd, oe = t.onWheel, Yt = t.onEnable, bt = t.onDisable, Ke = t.onClick, dt = t.scrollSpeed, me = t.capture, ee = t.allowClicks, Me = t.lockAxis, xe = t.onLockAxis;
    ...
    var Ct, De, ze, O, q, Xe, Ne, a = this, He = 0, gt = 0, St = t.passive || !Y && t.passive !== !1, U = Ot(l, Le), ht = Ot(l, ae), kt = U(), Ft = ht(), ce = ~u.indexOf("touch") && !~u.indexOf("pointer") && ot[0] === "pointerdown", Tt = wr(l), K = l.ownerDocument || Dt, et = [0, 0, 0], Ze = [0, 0, 0], _t = 0, lr = function() {
      ...
      var m = a.deltaX = Dn(et), F = a.deltaY = Dn(Ze), f = Math.abs(m) >= r, b = Math.abs(F) >= r;
    ...
    De = a._dc = ve.delayedCall(h || 0.25, Mr).pause(), a.deltaX = a.deltaY = 0, a._vx = dn(0, 50, !0), a._vy = dn(0, 50, !0), a.scrollX = U, a.scrollY = ht, a.isDragging = a.isGesturing = a.isPressed = !1, Zn(this), a.enable = function(S) {
...
var d, er, E, B, Ue, L, yn, Jr, Er, yr, dr, Yr, Ce, tn, gn, Ye, Rn, An, tr, ti, nn, ri, Oe, hn, ni, ii, Mt, _n, bn, ir, Cn, Qr, vn, on, Fr = 1, Se = Date.now, sn = Se(), je = 0, gr = 0, On = function(e, n, t) {
  ...
  var x = i.startColor, Y = i.endColor, M = i.fontSize, h = i.indent, c = i.fontWeight, v = B.createElement("div"), $ = Ut(t) || At(t, "pinType") === "fixed", X = e.indexOf("scroller") !== -1, ne = $ ? L : t, W = e.indexOf("start") !== -1, ge = W ? x : Y, k = "border-color:" + ge + ";font-size:" + M + ";color:" + ge + ";font-weight:" + c + ";pointer-events:none;white-space:nowrap;font-family:sans-serif,Arial;z-index:1000;padding:4px 8px;border-width:0;border-style:solid;";
  ...
  P.cache++, (e === !0 || !Ce && !ri && !B.fullscreenElement && !B.webkitFullscreenElement && (!hn || ii !== E.innerWidth || Math.abs(E.innerHeight - ni) > E.innerHeight * 0.25)) && Jr.restart(!0);
}, Vt = {}, Pi = [], pi = function o() {
  ...
  for (Ye = 0; Ye < T.length; Ye++)
    t = T[Ye], t && (!n || t._ctx === n) && (e ? t.kill(1) : t.revert(!0, !0));
  ...
  var t = Ot(e, n), r = "_scroll" + n.p2, i = function u(l, p, x, Y, M) {
  ...
  return e.init = function(t, r) {
    ...
    if (s._startClamp = s._endClamp = !1, s._dir = g, X *= 45, s.scroller = w, s.scroll = y ? y.time.bind(y) : ee, De = ee(), s.vars = t, r = r || t.animation, "refreshPriority" in t && (ti = 1, t.refreshPriority === -9999 && (Tr = s)), pt.tweenScroll = pt.tweenScroll || {
      ...
      min: 0.1,
      max: 2
    }, tt = Nt(tt) ? yr(tt.min, tt.max) : yr(tt, tt), rt = d.delayedCall(k.delay || It / 2 || 0.1, function() {
      var f = ee(), b = Se() - dt < 500, _ = Me.tween;
      if ((b || Math.abs(s.getVelocity()) < 10) && !_ && !tn && Ke !== f) {
        var C = (f - O) / U, fe = r && !ue ? r.totalProgress() : C, A = b ? 0 : (fe - Lt) / (Se() - dr) * 1e3 || 0, Z = d.utils.clamp(-C, 1 - C, Qt(A / 2) * A / 0.185), we = C + (k.inertia === !1 ? 0 : Z), V, H, z = k, it = z.onStart, G = z.onInterrupt, We = z.onComplete;
    ...
    if (s.previous = function() {
      ...
      if (!((Ce || !s.enabled) && !b)) {
        ...
        var fe = Yt(), A = bt(), Z = y ? y.duration() : ct(w, g), we = U <= 0.01 || !U, V = 0, H = C || 0, z = Nt(_) ? _.end : t.end, it = t.endTrigger || h, G = Nt(_) ? _.start : t.start || (t.start === 0 || !h ? 0 : c ? "0 0" : "0 100%"), We = s.pinnedContainer = t.pinnedContainer && Be(t.pinnedContainer, s), st = h && Math.max(0, T.indexOf(s)) || 0, he = st, _e, ye, Xt, Dr, be, se, lt, rn, Mn, ur, at, cr, Rr;
        for (Q && Nt(_) && (cr = d.getProperty(a, g.p), Rr = d.getProperty(He, g.p)); he-- > 0; )
          se = T[he], se.end || se.refresh(0, 1) || (Ce = s), lt = se.pin, lt && (lt === h || lt === c || lt === We) && !se.isReverted && (ur || (ur = []), ur.unshift(se), se.revert(!0, !0)), se !== T[he] && (st--, he--);
        for (ke(G) && (G = G(s)), G = On(G, "start", s), O = Hn(G, h, fe, g, ee(), Xe, a, s, A, j, Ee, Z, y, s._startClamp && "_startClamp") || (c ? -1e-3 : 0), ke(z) && (z = z(s)), $e(z) && !z.indexOf("+=") && (~z.indexOf(" ") ? z = ($e(G) ? G.split(" ")[0] : "") + z : (V = $r(z.substr(2), fe), z = $e(G) ? G : (y ? d.utils.mapRange(0, y.duration(), y.scrollTrigger.start, y.scrollTrigger.end, O) : O) + V, it = h)), z = On(z, "end", s), q = Math.max(O, Hn(z || (it ? "100% 0" : Z), it, fe, g, ee() + V, Ne, He, s, A, j, Ee, Z, y, s._endClamp && "_endClamp")) || -1e-3, V = 0, he = st; he--; )
        ...
        if (O += V, q += V, s._startClamp && (s._startClamp += V), s._endClamp && !Fe && (s._endClamp = q || -1e-3, q = Math.min(q, ct(w, g))), U = q - O || (O -= 0.01) && 1e-3, we && (me = d.utils.clamp(0, 1, d.utils.normalize(O, q, nt))), s._pinPush = H, Xe && V && (_e = {}, _e[g.a] = "+=" + V, We && (_e[g.p] = "-=" + ee()), d.set([Xe, Ne], _e)), c && !(vn && s.end >= ct(w, g)))
          _e = Qe(c), Dr = g === ae, Xt = ee(), Ze = parseFloat(K(g.a)) + H, !Z && q > 1 && (at = (ie ? B.scrollingElement || Ue : w).style, at = {
            style: at,
        ...
        }), s.start = O, s.end = q, De = ze = Fe ? nt : ee(), !y && !Fe && (De < nt && ee(nt), s.scroll.rec = 0), s.revert(!1, !0), dt = Se(), rt && (Ke = -1, rt.restart(!0)), Ce = 0, r && ue && (r._initted || zt) && r.progress() !== zt && r.progress(zt || 0, !0).render(r.time(), !0, !0), (we || me !== s.progress || y || $ || r && !r._initted) && (r && !ue && (r._initted || me || r.vars.immediateRender !== !1) && r.totalProgress(y && O < -1e-3 && !me ? d.utils.normalize(O, q, 0) : me, !0), s.progress = we || (De - O) / U === me ? 0 : me), c && v && (ce._pinOffset = Math.round(s.progress * _t)), D && D.invalidate(), isNaN(cr) || (cr -= d.getProperty(a, g.p), Rr -= d.getProperty(He, g.p), Hr(a, g, cr), Hr(Xe, g, cr - (C || 0)), Hr(He, g, Rr), Hr(Ne, g, Rr - (C || 0))), we && !Fe && s.update(), Y && !Fe && !St && (St = !0, Y(s), St = !1);
      }
      ...
      if (!(y && !_ && !f)) {
        ...
        if (b && (ze = De, De = y ? ee() : C, k && (Lt = Et, Et = r && !ue ? r.totalProgress() : A)), X && c && !Ce && !Fr && je && (!A && O < C + (C - ze) / (Se() - dr) * X ? A = 1e-4 : A === 1 && q > C + (C - ze) / (Se() - dr) * X && (A = 0.9999)), A !== Z && s.enabled) {
    ...
    if (E = window, B = document, Ue = B.documentElement, L = B.body, d && (Er = d.utils.toArray, yr = d.utils.clamp, _n = d.core.context || ut, nn = d.core.suppressOverwrites || ut, bn = E.history.scrollRestoration || "auto", xn = E.pageYOffset || 0, d.core.globals("ScrollTrigger", o), L)) {
      ...
      }), r.borderTopStyle = "solid", l = xt(L), ae.m = Math.round(l.top + ae.sc()) || 0, Le.m = Math.round(l.left + Le.sc()) || 0, i ? r.borderTopStyle = i : r.removeProperty("border-top-style"), t || (L.setAttribute("style", ""), L.removeAttribute("style")), Yr = setInterval(Xn, 250), d.delayedCall(0.5, function() {
        return Fr = 0;
      }), de(B, "touchcancel", ut), de(L, "touchstart", ut), Ir(de, B, "pointerdown,touchstart,mousedown", Fn), Ir(de, B, "pointerup,touchend,mouseup", Ln), gn = d.utils.checkPrefix("transform"), qr.push(gn), er = Se(), Jr = d.delayedCall(0.2, Ht).pause(), tr = [B, "visibilitychange", function() {
        var x = E.innerWidth, Y = E.innerHeight;
...
R.batch = function(o, e) {
  var n = [], t = {}, r = e.interval || 0.016, i = e.batchMax || 1e9, u = function(x, Y) {
    var M = [], h = [], c = d.delayedCall(r, function() {
      Y(M, h), M = [], h = [];
    }).pause();
    return function(v) {
...
var $n = function(e, n, t, r) {
  ...
  return h && d.set(h, {
    ...
    if (c.offset && g(), !Q)
      ...
      P.cache++;
      var j = W(), s, oe;
      t && (s = v(), oe = s + j * 0.05 * -I.velocityX / 0.227, j *= $n(v, s, oe, ct(x, Le)), w.vars.scrollX = y(oe)), s = c(), oe = s + j * 0.05 * -I.velocityY / 0.227, j *= $n(c, s, oe, ct(x, ae)), w.vars.scrollY = Te(oe), w.invalidate().duration(j).play(0.01), (Mt && w.vars.scrollY >= p || s >= p - 1) && d.to({}, {
        onUpdate: Pe,
        duration: j
      });
    }
    ...
    if ($t !== ge && Ie(), Q && t && v(y(s[2] === Q ? pt + (I.startX - I.x) : v() + Q - s[1])), j) {
      c.offset && g();
      var Yt = oe[2] === j, bt = Yt ? ie + I.startY - I.y : c() + j - oe[1], Ke = Te(bt);
...
//# sourceMappingURL=ScrollTrigger-7ivQEezo.js.map
