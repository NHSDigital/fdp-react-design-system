...
**Recent SPC Enhancements (post 30 Aug 2025):**

- **Embedded icon refactor**: Variation (and when applicable assurance) icons now rendered in a flexible top row outside the SVG for layout stability.
- **Assurance uncertainty behaviour**: Assurance icon is now suppressed (no placeholder glyph) when target lies inside the 3σ process band (capability uncertain) aligning with documented capability logic and passing regression test.
- **Gradient sequence washes**: Added contiguous run detection with absorption of singleton coloured points (and A–B–A pattern neutralisation) to render subtle gradient background washes for improvement / concern / common sequences without visual flicker or gaps.
- **Unified process line styling**: Forced neutral grey (#A6A6A6) process stroke to harmonise with common‑cause point colour and improve contrast consistency.
- **Trend orientation heuristic**: For suppressed single favourable points (no judgement icon) the fallback purple arrow now orients towards the favourable direction (Up/Down) improving semantic affordance.
- **Timeframe narration**: Automatic timeframe summarisation (days/weeks/months) injected when explicit context not provided, enriching live region output.
- **SPC tooltip overlay**: Upgraded to enriched `SPCTooltipOverlay` component (contextual narration hooks preserved) paving way for aggregated summaries.
- **Test reliability**: Fixed failing assurance uncertainty test (ensuring no assurance icon rendered during overlap state) – prevents regressions in capability logic.
