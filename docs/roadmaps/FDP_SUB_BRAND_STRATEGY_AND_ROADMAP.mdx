...
>
> - Brand infrastructure in place: CSS variable tokens at :root (NHS) with FDP overlays via `[data-brand="fdp"]`.
> - `BrandThemeProvider` implemented with document/local scope and controlled updates (brand prop sync fix).
> - Typography foundations complete: Source Sans Pro self-hosted, brand-aware font stacks, increased fluid clamps for legibility (NHS baselines), unitless line-heights, and letter-spacing presets.
> - Storybook updated: no global brand toggle; docs and playgrounds use local brand scope; font-debug CSS now respects brand variables.
> - Demos/docs: Typography docs, migration checklist, gradient swatches (including white→color variants), and an interactive Typography Playground with computed-style readout.
...
- Keep SSR first, clear progressive enhancement compatibility, and a11y standards.
...
## Runtime Theming: BrandThemeProvider

A minimal React provider sets the brand attribute on the document or on a local wrapper element.

- API: `<BrandThemeProvider brand="nhs" | "fdp" scope="document|local">{children}</BrandThemeProvider>`
...
- Hook: `useBrand()` returns `{ brand, setBrand }` when components must branch in TS (avoid where possible).
- Controlled updates: internal state stays in sync with the `brand` prop so toggles update immediately.
...
- Storybook
  - No global toolbar toggle. Brand switching is scoped per doc/page using a local `BrandThemeProvider` wrapper to avoid cross-story bleed.
  - Dedicated examples (e.g., Typography Playground) expose a brand toggle and show computed styles to verify the switch.
  - Use the accessibility addon to validate contrast and keyboard flows.

- Testing
  - Unit/Component: where relevant, add brand-parameterized smoke tests; verify roles/semantics remain correct across brands.
  - SSR: ensure correct data attribute during server render; no client-only refs in render.
  - Visual: optional per-brand snapshots for core set (Buttons, Inputs, Tables, Cards, Charts).
...
## Phase 0 – Scaffolding (PR1) ✅ Completed
...
- `dist/css/semantics.css` and `dist/css/brand-fdp.css` exist and contain variables. ✅

## Phase 1 – Theme infrastructure (PR2) ✅ Completed

1) BrandThemeProvider
- Export `<BrandThemeProvider brand="nhs|fdp" scope="document|local">` and `useBrand()`.
- SSR-safe: can set attribute server-side via prop. Controlled updates sync brand prop to state.

2) Storybook Brand Usage
- Local brand scope within docs/playgrounds; no global toolbar to keep isolation and predictability.

Acceptance
- Storybook launches and brand can be toggled locally within pages. ✅
- Sample pages show clear brand switch (e.g., typography font family and sizes). ✅

## Phase 2 – Pilot component conversions (PR3–PR5) – In Progress
