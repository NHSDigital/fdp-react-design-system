...
- [ ] Align partition start eligibility with SQL (remove/ignore transition buffer in parity)
- [ ] Tests around early partitions meeting minPoints
...
- [ ] When enabled, exclude MR outliers for MRbar AND recompute mean from outlier-excluded set
- [ ] Test fixture mirroring SQL reference
...
- [~] Curate fixture pack: XmR (stable, >3σ, two-of-three, shift, cross-partition trend), T (back-transform LCL ≤ 0 suppression), G (quantile limits)
...
Short term
- Centralise datasets and wire an expected‑colour table into the healthcare v2 story.
- Add parity fixture examples for T (LCL suppression after back‑transform) and G (quantile limits) with accompanying Storybook entries.
