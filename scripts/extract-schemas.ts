...
async function run() {
	const schemaFiles = await glob("src/components/**/**.schema.ts", {
		nocase: true,
	});
	const meta: ExtractedMetaV1 = {
		version:
			JSON.parse(await fs.promises.readFile("package.json", "utf-8")).version ||
			"0.0.0",
	...
	for (const file of schemaFiles) {
		...
		if (!candidates.length && isComponentSchema((mod as AnyModule).default)) {
			candidates.push([
				"default",
				(mod as AnyModule).default as ComponentSchema,
			]);
		}

		if (!candidates.length) {
			console.warn(
				"[extract-schemas] No ComponentSchema export found in",
				file
			);
			continue;
		}
		...
		} as ComponentSchema & { source: string };
