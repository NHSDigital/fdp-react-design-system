...
interface CheckboxesHandles { detach: () => void }
interface CheckboxesContainer extends HTMLElement { __nhsCheckboxesBehaviour?: CheckboxesHandles }
...
export function initCheckboxes(scope: ParentNode = document) {
  const groups = Array.from(scope.querySelectorAll<CheckboxesContainer>('.nhsuk-checkboxes'));
  groups.forEach(g => { if (!g.__nhsCheckboxesBehaviour) g.__nhsCheckboxesBehaviour = enhance(g); });
}

export function detachCheckboxes(scope: ParentNode = document) {
  const groups = Array.from(scope.querySelectorAll<CheckboxesContainer>('.nhsuk-checkboxes'));
  groups.forEach(g => { g.__nhsCheckboxesBehaviour?.detach(); delete g.__nhsCheckboxesBehaviour; });
}
...
if (typeof window !== 'undefined') {
  ...
  window.__nhsDetachCheckboxes = detachCheckboxes;
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => initCheckboxes()); else initCheckboxes();
  }
}
