...
<Meta title="Data Visualisation/Guides/Chart Alert Annotations" name="Overview" />
...
### Palette gradients (new)

If you want subtle fills or decorative washes consistent with the brand palette, you can also use the following gradient tokens:

- Light Blue: `gradient.palette.light-blue` — NHS Blue → `#60b2e2`
- Dark Blue: `gradient.palette.dark-blue` — NHS Blue → NHS Dark Blue
- Purple: `gradient.palette.purple` — `#ac98cb` → NHS Purple 20% tint
- Green: `gradient.palette.green` — NHS Green → NHS Dark Green (60% tint)
- Grey: `gradient.palette.grey` — NHS Dark Grey → NHS Mid Grey

Variants for direction:
- Vertical (top→bottom): `*-vertical` (e.g., `gradient.palette.dark-blue-vertical`)
- Diagonal (135°): `*-diagonal` (e.g., `gradient.palette.green-diagonal`)

Usage examples:

```tsx
// 1) Inline CSS background (e.g., card container)
<div style={{ background: tokens.gradient.palette["light-blue-diagonal"] }} />

// 2) Vertical bar background
<div style={{ background: tokens.gradient.palette["grey-vertical"] }} />

// 3) SVG fill
<rect fill={tokens.gradient.palette["dark-blue"]} />
```
