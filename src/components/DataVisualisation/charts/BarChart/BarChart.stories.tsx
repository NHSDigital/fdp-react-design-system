...
import { ChartRoot, LineScalesProvider, BandScalesProvider, Axis, GridLines, TooltipProvider, Legend, BarSeriesPrimitive, type BarSeries, useScaleContext } from '../../index';
...
export const StackedBars: Story = {
  ...
  parameters: { docs: { description: { story: 'Absolute stacked vertical bar chart using stackSeries output with BarSeriesPrimitive stacked prop.' } } }
};

export const StackedBarsPercent: Story = {
  render: () => {
    const categories = ['Cat A','Cat B','Cat C','Cat D','Cat E','Cat F','Cat G','Cat H','Cat I','Cat J','Cat K','Cat L'];
    const raw: BarSeries[] = Array.from({ length: 3 }).map((_, si) => ({
      id: `pb${si+1}`,
      data: categories.map(c => ({ x: c, y: Math.round(5 + Math.random()*25) }))
    }));
    const absStack = stackSeries(raw as any);
    const percentStack = normaliseStack(absStack.map(s => ({ ...s, stacked: s.stacked.map(d => ({ ...d })) })) as any);
    const margin = { left: 56, bottom: 48, right: 16, top: 12 };
    const Inner: React.FC = () => {
      const dims = useChartContext();
      const scale = useScaleContext();
    ...
    return (
      ...
      <ChartRoot width={760} height={320} margin={margin} ariaLabel="100 percent stacked bar chart (categorical)">
        <BandScalesProvider series={percentStack.map(s => ({ data: s.stacked.map(d => ({ x: d.x, y: d.y1 })) })) as any}>
          <TooltipProvider>
            <Inner />
            <Legend items={percentStack.map(s => ({ id: s.id, label: s.id }))} />
            <VisuallyHiddenLiveRegion />
          </TooltipProvider>
        </BandScalesProvider>
      </ChartRoot>
  ...
  parameters: { docs: { description: { story: '100% stacked vertical bar chart with percent y-axis.' } } }
};
...
export const StackedBarsToggle: Story = {
  render: () => {
    ...
    const percentStack = React.useMemo(()=> normaliseStack(absStack.map(s => ({ ...s, stacked: s.stacked.map(d => ({ ...d })) })) as any), [absStack]);
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontFamily: '"Frutiger W01", Frutiger, Arial, "Helvetica Neue", Helvetica, sans-serif' }}>
        ...
        <ChartRoot width={720} height={320} margin={margin} ariaLabel={`Stacked bar chart toggle (${mode})`}>
          <BandScalesProvider series={active.map(s => ({ data: s.stacked.map(d => ({ x: d.x, y: d.y1 })) })) as any}>
            <TooltipProvider>
              <Inner />
  ...
  // name removed (redundant)
  parameters: { docs: { description: { story: 'Interactive toggle between absolute and percent stacked bars.' } } }
};

export const GenericBars: Story = {
  render: () => {
    const categories = ['One','Two','Three','Four','Five','Six','Seven','Eight','Nine','Ten','Eleven','Twelve'];
  ...
  // name removed (redundant)
  parameters: { docs: { description: { story: 'Grouped bar chart with automatic width and spacing constraints.' } } }
};
...
export const ContinuousManyBarsScrollable: Story = {
  ...
  const requiredTotalWidth = computeContinuousBarChartTotalWidth(raw, { parseX: parse, barWidth: raw[0].barWidth, groupGap: gap, marginLeft: margin.left, marginRight: margin.right, minBarWidth: 12 });
    const Inner: React.FC = () => {
      const dims = useChartContext();
      if (!dims) return null;
      return (
  ...
  parameters: { docs: { description: { story: 'Continuous time-scale bars with horizontal scroll instead of width compression. Uses minBarWidth prop to avoid rendering bars below 12px (auto-expands total width when needed).' } } }
};
