...
function makeFlat(len: number, value: number): SPCDatum[] {
  ...
  return Array.from({ length: len }, (_, i) => ({ x: new Date(start + i*24*3600*1000), y: value }));
}
...
describe('SPCChart advanced behaviour', () => {
  ...
  test('detects shift special cause (XmR)', () => {
    ...
    const start = Date.UTC(2024,0,1);
    const data: SPCDatum[] = [];
    for (let i=0;i<10;i++) data.push({ x: new Date(start + i*86400000), y: 50 });
    for (let i=10;i<20;i++) data.push({ x: new Date(start + i*86400000), y: 60 });
    render(<SPCChart data={data} metricImprovement={ImprovementDirection.Up} />);
    const special = document.querySelectorAll('.fdp-spc__point--sc-improvement, .fdp-spc__point--sc-concern');
  ...
  test('T chart rare events produces limits after threshold', () => {
    ...
    const data: SPCDatum[] = base.map((v,i)=>({ x: new Date(start + i*86400000), y: v }));
    render(<SPCChart data={data} chartType="T" metricImprovement={ImprovementDirection.Up} />);
  ...
  test('G chart rare events flags extreme low gap as concern or improvement depending direction', () => {
    const start = Date.UTC(2024,0,1);
    const base = [12,11,13,12,14,13,12,11,12,13,12,11,1,13,12,11,12,13,12,11]; // single very low count between events
    const data: SPCDatum[] = base.map((v,i)=>({ x: new Date(start + i*86400000), y: v }));
    render(<SPCChart data={data} chartType="G" metricImprovement={ImprovementDirection.Up} />);
    const specials = document.querySelectorAll('.fdp-spc__point--sc-improvement, .fdp-spc__point--sc-concern');
