...
describe('SPCChart dataset metrics', () => {
  test.each(metricSeries.map(s => [s.metric, s.points.length]) as [string, number][])('renders %s with %d points', (metric, count) => {
  ...
  if (count >= 13) expect(cl.length).toBe(1); else expect(cl.length).toBe(0);
  });
});
