...
| `suppressIsolatedFavourablePoint` | true | Avoid classifying a single favourable 3σ point as “improvement” unless another rule corroborates (reduces over‑celebration) |
...
| `strictShewhartMode` | true | Disable all non-orthodox / visually forcing heuristic behaviours (comparative baseline emulation, retroactive neutralisation, emerging direction grace, suppression of isolated favourable 3σ) ensuring pure Shewhart interpretation |
...
<SPCChart
  ...
  settings={{ enableFourOfFiveRule: true, strictShewhartMode: true }}
/>
```

### Strict Shewhart Mode

`strictShewhartMode` (default `true`) enforces a conservative interpretation aligned with published NHS / IHI SPC literature. When enabled the engine:

- Ignores comparative baseline emulation (and all its invert / propagate / tail variants)
- Disables retroactive opposite-shift neutralisation
- Disables emerging direction grace precedence adjustments
- Leaves isolated favourable single 3σ points visible (does not suppress them)

Rationale: These heuristics can retrospectively relabel or hide original statistical evidence, risking over‑optimistic or narrative‑driven interpretation. Keeping strict mode on by default ensures charts remain transparent and reproducible under standard Shewhart rule sets. You can opt out (set `strictShewhartMode: false`) for exploratory/internal scenarios; provenance tags (if tooltips enabled) will then surface heuristic interventions.
