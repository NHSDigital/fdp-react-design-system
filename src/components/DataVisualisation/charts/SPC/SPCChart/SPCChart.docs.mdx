...
// import SPCChartStableWaitTimeExample from './logic/storybook/SPCChartStableWaitTimeExample';
// import SPCChartEarlyWaitTimeExample from './logic/storybook/SPCChartEarlyWaitTimeExample';
// import SPCChartIndividualsExample from './logic/storybook/SPCChartIndividualsExample';
// import SPCChartRareEventExample from './logic/storybook/SPCChartRareEventExample';
// import SPCChartPhasedBaselineExample from './logic/storybook/SPCChartPhasedBaselineExample';
// import SPCChartWarningsPanelExample from './logic/storybook/SPCChartWarningsPanelExample';
// import RunVsSPCTransitionExample from './logic/storybook/RunVsSPCTransitionExample';
...
If you need strict visual/data parity with historic SQL outputs (v2.6a conflict precedence and directional pruning), use the engine-level SQL compatibility wrapper. It replays per‑side ranking after the engine run and emits audit metadata (primeDirection, primeRank, primeRuleId, original values, pruned flag). UI toggles have been removed; SPCChart always reflects the native engine. See the guide: Data Visualisation → SPC → Guides → SPC SQL Compatibility (includes a deep‑dive on v2.6a conflict resolution).

Note on v2 engine settings: The v2 engine (logic_v2) also accepts a semantic hierarchical settings shape (`SpcSettingsHierarchical`) in addition to the flat v2.6a settings. Advanced consumers calling the v2 engine directly can import `normaliseSpcSettingsV2` to pre-normalise; `SPCChart` keeps its existing `SpcSettings` prop surface.
...
Optional rule toggles & thresholds are exposed via the `settings` prop (see Advanced settings). The four‑of‑five rule is OFF by default to align with the simplified NHS guidance and best practice; turn it on for more sensitive internal monitoring.
...
<Mermaid chart="graph TD
  A[<6 points] -->|Too few for SPC| B[Line / run chart only]
  B --> C[6-11 points]
  C -->|Run chart rules only| D[Initial SPC possible - mark limits as 'trial']
  D --> E[12-19 points]
  E -->|Establish / confirm SPC limits| F[20-30 points]
  F -->|Mature phase| G[>30 points] "
/>
...
### Gradient sequence transition strategies
...
<Table
  ...
  [ { code: 'enableFourOfFiveRule', codeLanguage: 'typescript' }, { text: 'Enable 4‑of‑5 beyond 1σ early warning rule.' }, { code: 'boolean', codeLanguage: 'typescript' }, { code: 'false', codeLanguage: 'typescript' } ],
...
```ts
import { buildSpc, SpcWarningSeverity, SpcWarningCategory } from './logic/spc';
