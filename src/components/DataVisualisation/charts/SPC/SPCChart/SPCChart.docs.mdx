...
import './SPCChart.docs.scss';
import SPCChartStableWaitTimeExample from './SPCChartStableWaitTimeExample';
import SPCChartEarlyWaitTimeExample from './SPCChartEarlyWaitTimeExample';
import SPCChartIndividualsExample from './SPCChartIndividualsExample';
import SPCChartRareEventExample from './SPCChartRareEventExample';
import SPCChartPhasedBaselineExample from './SPCChartPhasedBaselineExample';
import SPCChartWarningsPanelExample from './SPCChartWarningsPanelExample';
...
<Table
  ...
  rows={[
    ...
    [
      { text: 'Two-of-three' },
      { text: 'Two of the last three points in Zone A or beyond (between 2σ and 3σ, same side)' },
...
Why ±3σ? It empirically balances false alarms (Type I) against missed signals (Type II). Narrower bands over‑signal; wider bands under‑signal.
...
<SPCChart
  ...
  targets={[{ value: 20, label: '≤20 min target', kind: 'TARGET' }]}
  showZones
  enableRules
  showIcons
  processLineWidth={2}
...
<Table
  ...
  rows={[
    [
      ...
      { text: '{ x: Date | string | number; y: number; }[]' },
      { text: '–' },
    ],
    [
      { text: 'ariaLabel' },
    ...
    [
      { text: 'metricImprovement' },
      { text: 'Defines favourable direction for improvement/concern colouring.' },
      { text: 'ImprovementDirection' },
      { text: 'ImprovementDirection.Neither' },
    ],
    [
      { text: 'enableRules' },
    ...
    [
      ...
      { text: '{ severities?; categories?; codes?; }' },
      { text: '–' },
    ],
  ]}
  firstCellIsHeader
...
These map directly to the `SpcSettings` interface in `logic/spc.ts` (lines ~1–130). Defaults (in engine) shown in parentheses.
...
<Table
  ...
  rows={[
    [ { text: 'excludeMovingRangeOutliers' }, { text: 'Exclude extreme MR values when estimating sigma (XmR).' }, { text: 'boolean' }, { text: 'false' } ],
    [ { text: 'specialCauseShiftPoints' }, { text: 'Points needed on one side of mean to trigger shift rule.' }, { text: 'number' }, { text: '6' } ],
    [ { text: 'specialCauseTrendPoints' }, { text: 'Strict monotonic points needed to trigger trend rule.' }, { text: 'number' }, { text: '6' } ],
    [ { text: 'enableFourOfFiveRule' }, { text: 'Enable 4‑of‑5 beyond 1σ early warning rule.' }, { text: 'boolean' }, { text: 'false' } ],
    [ { text: 'minimumPoints' }, { text: 'Minimum non‑ghost points before limits/icons shown (global).' }, { text: 'number' }, { text: '13' } ],
    [ { text: 'minimumPointsWarning' }, { text: 'Emit warning when insufficient points for stable limits.' }, { text: 'boolean' }, { text: 'false' } ],
    [ { text: 'minimumPointsPartition' }, { text: 'Minimum points per partition (phase) for recalculated limits.' }, { text: 'number' }, { text: '12' } ],
    [ { text: 'maximumPointsPartition' }, { text: 'Hard cap per partition (subsequent points ignored for calc).' }, { text: 'number | null' }, { text: 'null' } ],
    [ { text: 'maximumPoints' }, { text: 'Global cap across whole series.' }, { text: 'number | null' }, { text: 'null' } ],
    [ { text: 'pointConflictWarning' }, { text: 'Warn when multiple raw rules tag same point conflictingly.' }, { text: 'boolean' }, { text: 'false' } ],
    [ { text: 'variationIconConflictWarning' }, { text: 'Warn when differing sustained indications would drive opposing icons.' }, { text: 'boolean' }, { text: 'true' } ],
    [ { text: 'nullValueWarning' }, { text: 'Warn if null/NaN values encountered (XmR/G).' }, { text: 'boolean' }, { text: 'false' } ],
    [ { text: 'targetSuppressedWarning' }, { text: 'Warn when targets ignored (e.g. T/G unsuited).' }, { text: 'boolean' }, { text: 'false' } ],
    [ { text: 'ghostOnRareEventWarning' }, { text: 'Warn on ghost rows in T/G charts (can distort spacing).' }, { text: 'boolean' }, { text: 'false' } ],
    [ { text: 'partitionSizeWarnings' }, { text: 'Warn if any partition too small for robust limits.' }, { text: 'boolean' }, { text: 'false' } ],
    [ { text: 'baselineSpecialCauseWarning' }, { text: 'Warn if baseline row itself exhibits special cause.' }, { text: 'boolean' }, { text: 'false' } ],
    [ { text: 'maximumPointsWarnings' }, { text: 'Warn when truncation caps applied.' }, { text: 'boolean' }, { text: 'false' } ],
    [ { text: 'assuranceCapabilityMode' }, { text: 'Classify assurance vs target using full process band (vs point only).' }, { text: 'boolean' }, { text: 'true' } ],
    [ { text: 'transitionBufferPoints' }, { text: 'Buffer window for simultaneous sustained shifts in opposite directions.' }, { text: 'number' }, { text: '2' } ],
    [ { text: 'collapseClusterRules' }, { text: 'Collapse co‑occurring cluster rules (keep strongest).' }, { text: 'boolean' }, { text: 'true' } ],
    [ { text: 'baselineSuggest' }, { text: 'Enable heuristic suggestions for new baselines.' }, { text: 'boolean' }, { text: 'false' } ],
    [ { text: 'baselineSuggestMinDeltaSigma' }, { text: 'Minimum |Δmean| (sigma units) to justify suggestion.' }, { text: 'number' }, { text: '0.5' } ],
    [ { text: 'baselineSuggestStabilityPoints' }, { text: 'Points of stability needed after candidate change.' }, { text: 'number' }, { text: '5' } ],
    [ { text: 'baselineSuggestMinGap' }, { text: 'Minimum points since previous accepted baseline.' }, { text: 'number' }, { text: '12' } ],
    [ { text: 'baselineSuggestScoreThreshold' }, { text: 'Minimum heuristic score to surface suggestion.' }, { text: 'number' }, { text: '50' } ],
    [ { text: 'autoRecalculateAfterShift' }, { text: 'Insert partition automatically after confirmed sustained shift.' }, { text: 'boolean' }, { text: 'false' } ],
    [ { text: 'autoRecalculateShiftLength' }, { text: 'Override length required for auto recalculation trigger.' }, { text: 'number' }, { text: 'inherits specialCauseShiftPoints' } ],
    [ { text: 'autoRecalculateDeltaSigma' }, { text: 'Minimum sigma‑scaled mean change for auto recalculation.' }, { text: 'number' }, { text: '0.5' } ],
  ]}
  firstCellIsHeader
/>
...
- Prefer default rule thresholds (6 / 6) unless your governance mandates alternatives.
- Turn on `enableFourOfFiveRule` for earlier (more sensitive) detection at cost of extra false positives.
- Use `excludeMovingRangeOutliers` sparingly; only when a single extreme swing inflates all future limits.
- Enable `baselineSuggest` in exploratory dashboards; keep it off for formal reporting to avoid “shifting the goalposts”.
- Use `autoRecalculateAfterShift` when you want automatic phase segmentation (removes need to manually supply `baselines`). Combine with `autoRecalculateDeltaSigma` to avoid over‑partitioning on negligible shifts.
- Leave `assuranceCapabilityMode` true to base Pass/Fail on predictable future performance, not a single latest point.
...
```ts
import { buildSpc, ChartType, ImprovementDirection, SpcWarningSeverity, SpcWarningCategory } from './logic/spc';
...
const errorWarnings = warnings.filter(w => w.severity === SpcWarningSeverity.Error);
```

### Surfacing warnings in the UI
...
Live warnings panel example (intentionally too few points & undersized partitions to surface warnings):
