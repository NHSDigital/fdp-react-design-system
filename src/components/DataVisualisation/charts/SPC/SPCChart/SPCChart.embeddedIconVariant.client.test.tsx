...
const data = Array.from({ length: 15 }, (_, i) => ({ x: i + 1, y: 50 + (i % 3) }));
...
describe('SPCChart embedded icon variant wiring', () => {
  it('renders classic (default) when no variant specified', () => {
    const { container } = render(<SPCChart data={data} metricImprovement={ImprovementDirection.Up} />);
    // Should include embedded icon wrapper
    expect(container.querySelector('.fdp-spc-chart__embedded-icon')).toBeTruthy();
  ...
  it('renders triangle variant when embeddedIconVariant="triangle"', () => {
    const { container } = render(<SPCChart data={data} metricImprovement={ImprovementDirection.Up} embeddedIconVariant="triangle" />);
  ...
  it('renders triangleWithRun variant with run dots when embeddedIconVariant="triangleWithRun" and run length provided', () => {
    const { container } = render(<SPCChart data={data} metricImprovement={ImprovementDirection.Up} embeddedIconVariant="triangleWithRun" embeddedIconRunLength={3} />);
    // Heuristic: triangleWithRun renders 5 small run circles (r=10). Count them.
    const runDots = Array.from(container.querySelectorAll('circle'))
