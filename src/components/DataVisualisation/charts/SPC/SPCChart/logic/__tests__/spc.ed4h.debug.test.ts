...
function toRows() {
  ...
  return ed4hValues.map((v, i) => ({ x: new Date(start.getFullYear(), start.getMonth() + i, 1), value: v }));
}
...
describe('ED4H classification regression guard', () => {
  ...
  const shiftHigh = result.rows.map(r => r.specialCauseShiftHigh);
  const shiftLow = result.rows.map(r => r.specialCauseShiftLow);
  const twoOfThreeHigh = result.rows.map(r => r.specialCauseTwoOfThreeAbove);
  const twoOfThreeLow = result.rows.map(r => r.specialCauseTwoOfThreeBelow);
  const trendInc = result.rows.map(r => r.specialCauseTrendIncreasing);
  const trendDec = result.rows.map(r => r.specialCauseTrendDecreasing);
  ...
  // even if a monotonic increasing trend segment begins while still below mean.
    const firstImprovementIndex = summary.findIndex(r => r.icon === 'improvement');
    ...
    if (firstImprovementIndex !== -1 && firstImprovementIndex < 12) {
      ...
      throw new Error('Unexpected early improvement at index ' + firstImprovementIndex + '\n' + JSON.stringify(summary, null, 2));
    }
