...
describe('SPC parity – suppressed favourable trend alignment', () => {
  ...
  const sql = buildSpcSqlCompat({ chartType: ChartType.XmR, metricImprovement: ImprovementDirection.Up, data: rows(values) });
    ...
    const firstImprovement = orthodoxIcons.findIndex(v=> v === VariationIcon.Improvement);
    ...
    orthodoxIcons.slice(0, firstImprovement).forEach(icon => expect(icon).not.toBe(VariationIcon.Improvement));
