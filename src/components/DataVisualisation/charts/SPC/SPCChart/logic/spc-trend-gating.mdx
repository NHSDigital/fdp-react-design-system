...
<Meta title="Data Visualisation/SPC/Guides/v1/Trend Gating Rationale"/>

# A Historic dive into Trend Side‑Gating Rationale, when in the SPC v1 logic it was optional

> Note (v2 engine default): SPCChart now uses the v2 SPC engine by default. Side‑gating of trend classification is always applied in the engine (classification layer). You can still choose visuals via `trendVisualMode` — `Ungated` (default visuals colour early trends by direction) or `Gated` (keep early trends neutral purple) — without changing the underlying classification. The examples below remain accurate conceptually; for v2 code, prefer importing from `logic_v2` (e.g. `buildSpcV26a`) and, for display classification that mirrors engine semantics, `computeSpcVisualCategories`.
...
```ts
import { buildSpcV26a as buildSpc } from '../../engine';
import { ChartType, ImprovementDirection } from '../../engine';
import { extractRuleIds } from '../descriptors';
