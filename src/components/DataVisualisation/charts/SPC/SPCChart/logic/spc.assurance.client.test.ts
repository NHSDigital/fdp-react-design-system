...
function withTarget(values: number[], target: number) {
	return values.map((v, i) => ({ x: i + 1, value: v, target }));
}
...
describe("SPC assurance icon capability mode", () => {
	...
	const seed = spcModule.buildSpc({
		...
		data: withTarget(baseValues, 0),
	});
	const last = seed.rows[seed.rows.length - 1];
	const lcl = last.lowerProcessLimit!;
	const ucl = last.upperProcessLimit!;
	...
	it("Up direction: pass when band above target, fail when band below target, none when overlaps", () => {
		const pass = spcModule.buildSpc({
			chartType: ChartType.XmR,
			metricImprovement: ImprovementDirection.Up,
			data: withTarget(baseValues, targetBelow),
		});
		const fail = spcModule.buildSpc({
			chartType: ChartType.XmR,
			metricImprovement: ImprovementDirection.Up,
			data: withTarget(baseValues, targetAbove),
		});
		const none = spcModule.buildSpc({
			chartType: ChartType.XmR,
			metricImprovement: ImprovementDirection.Up,
			data: withTarget(baseValues, targetInside),
		});
		const lp = pass.rows[pass.rows.length - 1]!;
		const lf = fail.rows[fail.rows.length - 1]!;
		const ln = none.rows[none.rows.length - 1]!;
	...
	it("Neither direction: assurance icon stays none even with target", () => {
		const neutral = spcModule.buildSpc({
			chartType: ChartType.XmR,
			metricImprovement: ImprovementDirection.Neither,
			data: withTarget(baseValues, targetInside),
		});
		const lastRow = neutral.rows[neutral.rows.length - 1]!;
