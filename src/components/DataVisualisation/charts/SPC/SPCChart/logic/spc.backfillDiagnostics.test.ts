...
describe("SPC special cause retroactive flag diagnostics", () => {
	it("reports any missing backfill situations", () => {
		...
		for (const tc of resolvedSpcTestCases) {
			...
			const record = (rule: string, indices: number[], context?: unknown) =>
				issues.push({ scenario: tc.title, rule, indices, context });

			// Shift High
			const firstShiftHigh = rows.findIndex((r) => r.specialCauseShiftUp);
			if (firstShiftHigh >= 0) {
				...
				if (windowIdx.length === SHIFT_N) {
					const missing = windowIdx.filter((i) => !rows[i].specialCauseShiftUp);
					if (missing.length)
						record("shiftHigh_missing_backfill", missing, { windowIdx });
				}
			}
			// Shift Low
			const firstShiftLow = rows.findIndex((r) => r.specialCauseShiftDown);
			if (firstShiftLow >= 0) {
				const windowIdx = [] as number[];
				let count = 0;
				...
				if (windowIdx.length === SHIFT_N) {
					const missing = windowIdx.filter(
						(i) => !rows[i].specialCauseShiftDown
					);
					if (missing.length)
						record("shiftLow_missing_backfill", missing, { windowIdx });
				}
			}
			...
			// Trend Increasing
			const firstTrendInc = rows.findIndex((r) => r.specialCauseTrendUp);
			if (firstTrendInc >= 0) {
				const windowIdx = [] as number[];
				let prevVal: number | null = null;
				for (
					let back = firstTrendInc;
					back >= 0 && windowIdx.length < TREND_N;
					back--
				) {
					const r = rows[back];
					if (!r.ghost && typeof r.value === "number") {
						if (prevVal === null || r.value < prevVal) {
							// walking backwards ensures strictly increasing forward
							windowIdx.unshift(back);
							prevVal = r.value;
				...
				if (windowIdx.length === TREND_N) {
					const missing = windowIdx.filter((i) => !rows[i].specialCauseTrendUp);
					if (missing.length)
						record("trendIncreasing_missing_backfill", missing, { windowIdx });
				}
			}
			// Trend Decreasing
			const firstTrendDec = rows.findIndex((r) => r.specialCauseTrendDown);
			if (firstTrendDec >= 0) {
				const windowIdx = [] as number[];
				let prevVal: number | null = null;
				for (
					let back = firstTrendDec;
					back >= 0 && windowIdx.length < TREND_N;
					back--
				) {
					const r = rows[back];
				...
				if (windowIdx.length === TREND_N) {
					const missing = windowIdx.filter(
						(i) => !rows[i].specialCauseTrendDown
					);
					if (missing.length)
						record("trendDecreasing_missing_backfill", missing, { windowIdx });
				}
			}
		}
