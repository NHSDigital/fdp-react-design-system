...
describe("baseline suggestion heuristic", () => {
	it("suggests a baseline at the start of a sustained shift", () => {
		...
		expect(r.suggestedBaselines && r.suggestedBaselines.length).toBeGreaterThan(
			0
		);
		...
		expect(
			first.reason === BaselineSuggestionReason.Shift ||
				first.reason === BaselineSuggestionReason.Point ||
				first.reason === BaselineSuggestionReason.Trend
		).toBe(true);
	...
	it("may suggest a baseline after a trend consolidates (trend or shift)", () => {
		...
		if (r.suggestedBaselines && r.suggestedBaselines.length) {
			...
			expect(
				r.suggestedBaselines.some(
					(s) =>
						s.reason === BaselineSuggestionReason.Trend ||
						s.reason === BaselineSuggestionReason.Shift ||
						s.reason === BaselineSuggestionReason.Point
				)
			).toBe(true);
		}
	});
