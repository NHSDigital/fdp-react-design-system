...
describe("SPC baseline special cause warning", () => {
	it("emits baseline_with_special_cause when baseline row is special cause single point", () => {
		...
		const data = [
			{ x: 1, value: baselineVal, baseline: true },
			...rest.map((v, i) => ({ x: i + 2, value: v })),
		];
		const { warnings, rows } = spcModule.buildSpc({
		...
		expect(warnings.some((w) => w.code === "baseline_with_special_cause")).toBe(
			true
		);
	});
});
