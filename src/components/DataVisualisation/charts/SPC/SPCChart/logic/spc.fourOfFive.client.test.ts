...
describe('SPC four-of-five rule', () => {
  it('flags when enabled and not when disabled', () => {
    const enabled = spcModule.buildSpc({ chartType: ChartType.XmR, metricImprovement: ImprovementDirection.Up, data, settings:{ enableFourOfFiveRule:true } });
    const disabled = spcModule.buildSpc({ chartType: ChartType.XmR, metricImprovement: ImprovementDirection.Up, data, settings:{ enableFourOfFiveRule:false } });
  ...
  expect(lastFiveDisabled.every(r => !r.specialCauseFourOfFiveUp)).toBe(true);
  });
});
