...
function loadCsv(): number[] {
	const csvPath = path.resolve(
		__dirname,
		"../test-data/Data - For testing.csv"
	);
...
describe("SPC XmR golden dataset parity", () => {
	it("matches expected mean, MR-bar and limits and flags special-cause points", () => {
		...
		expect(Math.abs((last.upperProcessLimit ?? 0) - expectedUcl)).toBeLessThan(
			tol
		);
		expect(Math.abs((last.lowerProcessLimit ?? 0) - expectedLcl)).toBeLessThan(
			tol
		);

		// Special cause: final declining tail drops below lower limit for rows with values < LCL.
		const belowLimitRows = rows.filter(
			(r) =>
				r.value !== null &&
				r.lowerProcessLimit !== null &&
				(r.value as number) < (r.lowerProcessLimit as number)
		);
		const belowIds = belowLimitRows.map((r) => r.rowId);
		// In dataset: values 35,30,25 are below LCL
		expect(belowIds).toEqual([22, 23, 24]);
		belowLimitRows.forEach((r) =>
			expect(r.specialCauseSinglePointDown).toBe(true)
		);

		// Trend: Expect decreasing trend flagged on last two rows (once >= 6 strictly decreasing points collected)
		const trendDecreasingIds = rows
			.filter((r) => r.specialCauseTrendDown)
			.map((r) => r.rowId);
