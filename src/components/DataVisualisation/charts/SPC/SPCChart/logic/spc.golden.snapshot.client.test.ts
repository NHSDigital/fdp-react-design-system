...
interface GoldenRow {
  rowId: number; value: number | null; ghost: boolean; mean: number | null; mr: number | null; mrMean: number | null;
  ucl: number | null; lcl: number | null; u2: number | null; l2: number | null; u1: number | null; l1: number | null;
  vIcon: string; sc3High: boolean; sc3Low: boolean; sc23High: boolean; sc23Low: boolean; sc45High: boolean; sc45Low: boolean;
  shiftHigh: boolean; shiftLow: boolean; trendUp: boolean; trendDown: boolean;
}
...
const datasets = {
  XmR: { chartType: 'XmR' as const, data: [55,60,51,50,51,60,50,51,52,52,60,50,60,50,49,45,51,45,44,43,40,35,30,25].map((v,i)=>({x:i+1,value:v})) },
  T: { chartType: 'T' as const, data: [5,7,9,4,6,8,30,7,6,5,4,3,2,1].map((v,i)=>({x:i+1,value:v,target:10, ghost: i===5})) },
  G: { chartType: 'G' as const, data: [10,11,9,10,12,8,7,6,40,7,8,9,10,11].map((v,i)=>({x:i+1,value:v,target:10, ghost: i===4})) },
};

function project(rows: any[]): GoldenRow[] { return rows.map(r => ({
  rowId: r.rowId,
  value: r.value,
...
const NUM_FIELDS: (keyof GoldenRow)[] = ['mean','mr','mrMean','ucl','lcl','u2','l2','u1','l1'];
const TOL = 1e-9;
  ...
  (['XmR','T','G'] as const).forEach(key => {
    it(`${key} chart rows match golden snapshot`, () => {
      ...
      for (let i=0;i<expected.length;i++) {
        ...
        ['sc3High','sc3Low','sc23High','sc23Low','sc45High','sc45Low','shiftHigh','shiftLow','trendUp','trendDown'].forEach(flag => {
          // @ts-ignore
          expect(act[flag]).toBe(exp[flag]);
        });
      }
    });
  });
