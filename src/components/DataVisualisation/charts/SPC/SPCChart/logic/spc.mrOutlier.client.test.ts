...
const values = [
	100, 101, 99, 100, 100, 100, 100, 130, 100, 101, 99, 100, 100, 100, 100, 101,
	99, 100,
];
...
describe("SPC moving range outlier exclusion", () => {
	it("reduces MR mean and tightens limits when exclusion enabled", () => {
		const noExcl = spcModule.buildSpc({
			chartType: ChartType.XmR,
			metricImprovement: ImprovementDirection.Up,
			data,
			settings: { excludeMovingRangeOutliers: false },
		});
		const excl = spcModule.buildSpc({
			chartType: ChartType.XmR,
			metricImprovement: ImprovementDirection.Up,
			data,
			settings: { excludeMovingRangeOutliers: true },
		});
		const lastNo = noExcl.rows[noExcl.rows.length - 1]!;
		const lastEx = excl.rows[excl.rows.length - 1]!;
		...
		expect((lastEx.mrMean ?? Infinity) <= (lastNo.mrMean ?? -Infinity)).toBe(
			true
		);
