...
import { buildSpc, ChartType, ImprovementDirection, AssuranceIcon } from './spc';

const toRows = (vals: number[], target?: number) => vals.map((v,i)=> ({ x: i+1, value: v, target }));
...
describe('SPC baseline recalculation crossing & assurance boundary equality', () => {
  ...
  const high = [80,81,82,83,82,81,84,83,85,84]; // extended to ensure >= shiftN sustained points
    ...
    const { rows } = buildSpc({
      ...
      data: toRows(values),
      settings: { autoRecalculateAfterShift: true, autoRecalculateDeltaSigma: 0.3, autoRecalculateShiftLength: 8 }
    });
  ...
  const prevPartitionRows = rows.filter(r => r.partitionId === rows[0].partitionId);
  const newPartitionRows = rows.filter(r => r.partitionId === rows[recalcIndex].partitionId);
  ...
  it('assurance icon boundary: value exactly at target yields Pass for Up metric with single-point fallback mode', () => {
    ...
    const vals = [100,101,99,100,101,100,100,101,100,101,100,101,100];
