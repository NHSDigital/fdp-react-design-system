...
describe("SPC trend gating – alternating equal/above & mixed-direction partial trends", () => {
	it("Alternating equal & above-mean pattern does not yield premature Improvement until true high-side shift", () => {
		...
		const values = [
			100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 101, 100,
			101, 100, 101, 110, 111, 112, 113, 114, 115,
		];
		const { rows } = buildSpc({
	...
	it("Mixed-direction partial trends (increase then decrease while above mean) do not create false Concern in Up metric", () => {
		...
		const values = [
			...
			53,
			54,
			55, // initial climb (likely below mean early)
			60,
			62,
			64,
			66,
			68,
			70, // strong increase crossing mean -> improvement region
			69,
			68,
			67,
			66,
			65,
			64, // gentle decrease but remain relatively high
			75,
			78,
			80, // re-acceleration
		];
		...
		expect(
			rows.some((r) => r.variationIcon === VariationIcon.Improvement)
		).toBe(true);
	});
});
