...
describe('SPC trend gating – cross-mean & equal-to-mean interruptions', () => {
  it('Increasing trend that straddles mean: only post-cross points become Improvement (Up direction)', () => {
    ...
    const values = [
      50,51,50,52,51, // noisy low baseline
      60,62,64,66,68,70, // monotonic increase (length 6) starts below eventual mean, ends above
      72,73,74,75,74,73 // sustained high
    ];
    const { rows } = buildSpc({ chartType: ChartType.XmR, metricImprovement: ImprovementDirection.Up, data: toRows(values), settings:{ minimumPoints:15 }});
    ...
    const below: number[] = []; const above: number[] = [];
    for (let i=segmentStart;i<=segmentEnd;i++) {
      const r = rows[i]!;
      if (r.value! > meanVal) above.push(i); else if (r.value! < meanVal) below.push(i); else {/* equal not expected here */}
    }
    ...
    const anyAboveImprovement = above.some(i => rows[i]!.variationIcon === VariationIcon.Improvement);
  ...
  it('Equal-to-mean value breaks shift/trend continuity and is not classified as Improvement while below-mean trend still forming', () => {
    ...
    const { rows } = buildSpc({ chartType: ChartType.XmR, metricImprovement: ImprovementDirection.Up, data: toRows(values), settings:{ minimumPoints:11 }});
    ...
    const firstImprovementIdx = rows.findIndex(r => r.variationIcon === VariationIcon.Improvement);
