...
describe('SPC trend side gating – additional edge cases', () => {
  ...
  const { rows } = buildSpc({ chartType: ChartType.XmR, metricImprovement: ImprovementDirection.Up, data: toRows(values), settings:{ enableFourOfFiveRule:false, minimumPoints:12 }});
  ...
  const meanVal = rows[0].mean!;
    ...
    const crossing = rows.findIndex(r => r.value !== null && r.mean !== null && r.value > r.mean);
    ...
    const postImprovement = rows.slice(crossing).some(r => r.variationIcon === VariationIcon.Improvement);
