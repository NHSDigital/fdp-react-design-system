...
describe("SPC trend side gating (regression)", () => {
	it("Down direction: monotonic decreasing trend above mean should NOT yield Improvement until values cross mean", () => {
		...
		const values = [
			...
			87,
			86,
			85, // monotonic decreasing trend (should not yet be Improvement)
			89,
			88,
			87,
			88,
			87,
			89, // mixed baseline noise (above mean, maintains high-side run)
			78,
			77,
			76,
			75,
			...
			75,
			73,
			74, // low shift + consolidation (should be Improvement side)
		];
		...
		const firstLowIdx = rows.findIndex(
			(r) => r.mean !== null && r.value !== null && r.value < r.mean
		);
	...
	it("Neutral direction: trend signals never map to Improvement/Concern (always Neither)", () => {
		...
		const trendRows = rows.filter(
			(r) => r.specialCauseTrendUp || r.specialCauseTrendDown
		);
