...
const baseRow = (): SpcRow => ({
	...
	variationIcon: VariationIcon.Neither,
	assuranceIcon: AssuranceIcon.None,
	upperBaseline: null,
	lowerBaseline: null,
	movingRangeHighPointValue: null,
	ghostValue: null,
	ghostFlag: false,
...
describe("spcDescriptors", () => {
	...
	it("extractRuleIds returns rule ids for set flags", () => {
		const r = baseRow();
		r.specialCauseSinglePointUp = true;
		r.specialCauseTrendDown = true;
		expect(extractRuleIds(r)).toEqual([
			SpcRuleId.SinglePointUp,
			SpcRuleId.TrendDown,
		]);
		// Ensure glossary entries exist
		expect(ruleGlossary[SpcRuleId.SinglePointUp].tooltip).toBeTruthy();
	});
