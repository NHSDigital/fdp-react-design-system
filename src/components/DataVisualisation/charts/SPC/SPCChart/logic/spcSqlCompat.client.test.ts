...
import { ChartType, ImprovementDirection, VariationIcon } from './spc';
...
describe('spcSqlCompat directional pruning', () => {
  it('produces variation icons consistent with pruned values (improvement metric Up)', () => {
    const data = makeData([100,101,102,103,104,105, 95,96,97,98,99, 110]);
    const res = buildSpcSqlCompat({ chartType: ChartType.XmR, metricImprovement: ImprovementDirection.Up, data, disableTrendSideGating: true });
    expect(res.rows.length).toBe(data.length);
    // Ensure no runtime errors and that icons are assigned
    const last = res.rows[res.rows.length -1];
    expect([VariationIcon.Improvement, VariationIcon.Concern, VariationIcon.Neither]).toContain(last.variationIcon);
  });

  it('does not error under potential rank tie scenario (improvement bias opportunistic)', () => {
    const data = makeData([100,100,100,100,100,100, 100.1,100.2,100.3,100.4,100.5,100.6]);
    const res = buildSpcSqlCompat({ chartType: ChartType.XmR, metricImprovement: ImprovementDirection.Up, data, disableTrendSideGating: true });
