...
import { buildSpcSqlCompat, PrimeDirection } from './spcSqlCompat';
import { ChartType, ImprovementDirection, SpcRuleId } from './spc';
...
describe('spcSqlCompat primeRuleId selection', () => {
  it('selects highest ranked rule on winning side when multiple present', () => {
    ...
    const data = Array.from({ length: 20 }).map((_, i) => ({ x: i, value: i < 5 ? 10 : 10 + i }));
    const { rows } = buildSpcSqlCompat({ chartType: ChartType.XmR, metricImprovement: ImprovementDirection.Up, data, disableTrendSideGating: true });
    const last = rows[rows.length - 1];
    // If both a trend and shift are detected upwards, primeRuleId should be Trend (highest rank 4) under winning Up side.
    if (last.specialCauseTrendIncreasing && last.specialCauseShiftHigh) {
  ...
  it('falls back to lower rank when higher one absent', () => {
    ...
    const data = [
      // baseline stable
      { x: 0, value: 10 }, { x: 1, value: 10 }, { x: 2, value: 10 }, { x: 3, value: 10 }, { x: 4, value: 10 },
      // shift upwards (sequence of points above baseline mean without monotonic increase)
      { x: 5, value: 15 }, { x: 6, value: 15 }, { x: 7, value: 15 }, { x: 8, value: 15 }, { x: 9, value: 15 }, { x: 10, value: 15 }, { x: 11, value: 15 }, { x: 12, value: 15 },
    ];
    const { rows } = buildSpcSqlCompat({ chartType: ChartType.XmR, metricImprovement: ImprovementDirection.Up, data, disableTrendSideGating: true });
    const last = rows[rows.length - 1];
    if (last.specialCauseShiftHigh && !last.specialCauseTrendIncreasing) {
