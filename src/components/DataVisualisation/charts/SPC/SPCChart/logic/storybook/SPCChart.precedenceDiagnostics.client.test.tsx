...
function makeProgressiveDeclineWithNoise() {
	...
	const values = [
		...
		11.1,
		11.05,
		11.15, // slight uptick breaks monotonic early
		11.0,
		10.92,
		10.88,
		10.9, // another tiny uptick
		10.75,
		10.7,
		10.62,
		10.6,
...
describe("SPC precedence diagnostics (decline inside high-side shift)", () => {
	it("logs rows where downward movement still classified as concern", () => {
		...
		const diagnostics = result.rows.map((r, i) => ({
			...
			trendDecreasing: r.specialCauseTrendDown,
			twoOfThreeAbove: r.specialCauseTwoOfThreeUp,
		...
		const conflicting = diagnostics.filter(
			(d) =>
				d.shiftHigh &&
				!d.trendDecreasing &&
				d.localSlope < 0 &&
				d.variation === VariationIcon.Concern
		);
