...
function expectImprovementSide(icon: VariationIcon, dir: ImprovementDirection) {
  if (dir === ImprovementDirection.Up) {
    expect([VariationIcon.ImprovementHigh, VariationIcon.CommonCause]).toContain(icon);
  } else if (dir === ImprovementDirection.Down) {
    expect([VariationIcon.ImprovementLow, VariationIcon.CommonCause]).toContain(icon);
  }
}
...
describe('logic_v2 healthcare parity (XmR)', () => {
  it('computes last-point judgement on the correct improvement side (or common cause)', () => {
    for (const ds of datasets) {
      const rows = buildSpcV26a({ chartType: ChartType.XmR, metricImprovement: ds.dir, data: asInput(ds.values), settings: { minimumPoints: 12 } }).rows;
  ...
  it('flags at least one special-cause event for datasets described as having a signal', () => {
    ...
    for (const ds of datasets.filter(d => namesWithSignals.includes(d.name))) {
      const rows = buildSpcV26a({ chartType: ChartType.XmR, metricImprovement: ds.dir, data: asInput(ds.values), settings: { minimumPoints: 12 } }).rows;
