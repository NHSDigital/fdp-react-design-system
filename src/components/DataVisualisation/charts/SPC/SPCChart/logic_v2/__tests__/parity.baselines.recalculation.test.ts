...
function asInput(values: number[], baselineIndex: number) {
  return values.map((v, i) => ({ x: i + 1, value: v, ghost: false, baseline: i === baselineIndex, target: null }));
}
...
describe('logic_v2: baselines → recalculation and partitioned rules', () => {
  it('recalculates mean and limits per partition when a baseline is applied', () => {
    ...
    expect(Math.abs((last1.upperProcessLimit as number) - (last2.upperProcessLimit as number))).toBeGreaterThan(0);
    expect(Math.abs((last1.lowerProcessLimit as number) - (last2.lowerProcessLimit as number))).toBeGreaterThan(0);
  });
  ...
  it('does not allow shift/two-of-three windows to cross a baseline boundary (partitioned rules)', () => {
    ...
    const rows = buildSpcV26a({
      ...
      metricImprovement: ImprovementDirection.Up,
      data: asInput(series, baselineAt),
      settings: withParityV26({ minimumPoints: 2, shiftPoints: 4, twoSigmaIncludeAboveThree: true }),
