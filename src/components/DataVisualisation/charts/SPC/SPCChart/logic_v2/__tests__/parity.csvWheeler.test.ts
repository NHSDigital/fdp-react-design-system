...
describe("logic_v2: CSV parity (Wheeler 2.66×MR̄ limits)", () => {
	it("matches Wheeler UPL/LPL and mean from the provided CSV", () => {
		const csvPath = path.resolve(
			process.cwd(),
			"src/components/DataVisualisation/charts/SPC/SPCChart/test-data/Data - For testing.csv"
		);
		const csv = fs.readFileSync(csvPath, "utf-8");
		const {
			vals,
			mean: expectedMean,
			mrMean: expectedMrMean,
			sigma: expectedSigma,
			wheelerUpl,
			wheelerLpl,
		} = parseCsv(csv);
		...
		expect(
			Math.abs((last.upperProcessLimit as number) - wheelerUpl)
		).toBeLessThan(tol);
		expect(
			Math.abs((last.lowerProcessLimit as number) - wheelerLpl)
		).toBeLessThan(tol);
	});
});
