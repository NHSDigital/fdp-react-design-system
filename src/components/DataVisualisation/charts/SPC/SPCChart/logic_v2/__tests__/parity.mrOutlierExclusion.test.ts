...
describe('parity: MR outlier exclusion recomputes MRbar and mean', () => {
  it('excludes value(s) whose MR exceed raw MR UCL when computing centre line; and trims MR outliers for MRbar', () => {
    ...
    const series = [10, 10.2, 9.9, 10.1, 40, 10.0, 9.8, 10.1, 10.0, 9.9, 10.2, 10.1, 9.9];

    const base = buildSpcV26a({
      chartType: ChartType.XmR,
      metricImprovement: ImprovementDirection.Neither,
      data: asInput(series),
      settings: withParityV26({ excludeMovingRangeOutliers: false, minimumPoints: 12 }),
    }).rows;
    ...
    const excluded = buildSpcV26a({
      ...
      metricImprovement: ImprovementDirection.Neither,
      data: asInput(series),
      settings: withParityV26({ excludeMovingRangeOutliers: true, minimumPoints: 12 }),
    }).rows;
    ...
    if (lastEx.upperProcessLimit !== null && lastEx.mean !== null && lastBase.upperProcessLimit !== null && lastBase.mean !== null) {
