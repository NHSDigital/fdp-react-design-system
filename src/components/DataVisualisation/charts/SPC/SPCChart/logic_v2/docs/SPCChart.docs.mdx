...
import SPCChartStableWaitTimeExample from '../doc-examples/SPCChartStableWaitTimeExample';
import SPCChartEarlyWaitTimeExample from '../doc-examples/SPCChartEarlyWaitTimeExample';
import SPCChartIndividualsExample from '../doc-examples/SPCChartIndividualsExample';
import SPCChartRareEventExample from '../doc-examples/SPCChartRareEventExample';
import SPCChartPhasedBaselineExample from '../doc-examples/SPCChartPhasedBaselineExample';
import SPCChartWarningsPanelExample from '../doc-examples/SPCChartWarningsPanelExample';
import TrendGatingExample from '../doc-examples/TrendGatingExample';
...
import {
  ...
  SPCVariationIcon,
  SPCAssuranceIcon,
} from '@nhsdigital/nhs-fdp-design-system/components/DataVisualisation/charts/SPC';
```
...
Notes:
- The SPC barrel exports icons and descriptor utilities intended for UI use. Engine internals for v2 live under logic_v2 and are not part of the public barrel surface.
- SPCChart supports SSR; use your usual SSR setup and include CSS either globally or per‑component.

### Props types and enums

Types and enums for SPCChart props (for example `TrendVisualMode`, `SequenceTransition`, `SPCChartProps`, `SPCDatum`) are defined alongside the component in `SPCChart.props.ts` and are not re‑exported from the component file. Import them directly from the props module:
...
import {
  ...
  type SPCChartProps,
  type SPCDatum,
} from '@nhsdigital/nhs-fdp-design-system/components/DataVisualisation/charts/SPC/SPCChart/SPCChart.props';
```
...
Advanced: The v2 engine accepts a semantic hierarchical settings shape (`SpcSettingsHierarchical`) as well as the classic flat v2.6a settings. If you call the v2 engine directly, prefer the helpers in `logic_v2` (not exported from the public barrel). The `SPCChart` prop surface remains unchanged (`SpcSettings`).

## Grouped props (preferred) and migration

SPCChart now supports convenient grouped prop blocks that make intent clearer and reduce top‑level prop noise. Flat props continue to work (backwards compatible), but examples below prefer grouped blocks. When both are provided, grouped values take precedence.

- input: data/targets/baselines/ghosts
- engine: chartType/metricImprovement/settings
- visualsEngine: scenario/settings (advanced presets/overlays)
- a11y: label/announceFocus/unit/narrationContext
- container: height/className
- meta: source (below‑chart text)

Example (grouped, recommended):

```tsx
import { SPCChart } from './SPCChart';
import { ImprovementDirection, ChartType } from './types';

const series = [36, 37, 34, 35].map((y, i) => ({ x: new Date(2025, 0, i + 1), y }));

<SPCChart
  input={{ data: series }}
  engine={{ chartType: ChartType.XmR, metricImprovement: ImprovementDirection.Down }}
  a11y={{ label: 'Clinic wait time SPC chart', unit: 'minutes' }}
  ui={{ visuals: { showZones: true, gradientSequences: true } }}
...
Notes:
- Dev‑time warnings may guide you to the grouped syntax if you pass deprecated flat props.
- You can mix approaches during migration; grouped fields win when both are specified.

### Sharing a single engine run with SPCSpark (precomputed visuals)

For perfect colour parity between the compact `SPCSpark` and the full `SPCChart`, compute the SPC engine once for your series and pass the engine results to both components:

- To the chart via `precomputed={{ rows, visuals }}`
- To the spark via `visualCategories={visuals}` (and optionally series state for icons / narration)

This avoids duplicate engine work and ensures both components classify and colour points identically, including neutral “No Judgement” cases.

Recommended helper (DRY):

```tsx
import { computeSpcPrecomputed, ImprovementDirection, ChartType, DEFAULT_MIN_POINTS } from '@nhsdigital/nhs-fdp-design-system/components/DataVisualisation/charts/SPC';
...
<>
  <SPCChart
    ariaLabel="A&E 4h performance"
    data={values.map((y, i) => ({ x: i, y }))}
    engine={{ metricImprovement: ImprovementDirection.Up }}
    precomputed={{ rows: summary.rows, visuals: summary.visuals }}
    showPoints
  />
  <SPCSpark
    data={values.map((v, i) => ({ x: i, value: v }))}
    metricImprovement={ImprovementDirection.Up}
...
```tsx
import { buildSpcV26a, computeSpcVisualCategories, ChartType, ImprovementDirection } from '../engine';
import { SPCChart } from '../SPCChart/SPCChart';
import { SPCSpark } from '../SPCSpark/SPCSpark';

const values = [60,61,60,61,60,61,60,61,60, 78,79,80,81,82,81,83,82,81];
const data = values.map((v, i) => ({ x: i, value: v }));
const engine = buildSpcV26a({ chartType: ChartType.XmR, metricImprovement: ImprovementDirection.Up, data, settings: { minimumPoints: DEFAULT_MIN_POINTS, chartLevelEligibility: true } });
const rows = engine.rows;
const visuals = computeSpcVisualCategories(rows, { metricImprovement: ImprovementDirection.Up, enableNeutralNoJudgement: true });
...
- `precomputed` (SPCChart): object with shape `{ rows: V2Row[]; visuals: SpcVisualCategory[] }`
- `visualCategories` (SPCSpark): `SpcVisualCategory[]`

#### Standalone icon parity using the same precompute

If you want a standalone icon (e.g. in a table cell or summary card) that exactly matches the embedded chart icon, pass the same `summary` to `SPCVariationIcon`:

```tsx
import { SPCVariationIcon } from '../SPCIcons/SPCIcon';

<SPCVariationIcon precomputed={summary} improvementDirection={ImprovementDirection.Up} />
```

This consumes the engine's last-row `variationIcon` and orients lettering consistently with `metricImprovement`.
...
// Enum is defined in ./types.ts (public UI surface)
...
import RunVsSPCTransitionExample from '../doc-examples/RunVsSPCTransitionExample';
...
<SPCChart
  input={{ data: waitTimes, targets: Array.from({ length: waitTimes.length }, () => 20) }}
  engine={{ chartType: ChartType.XmR, metricImprovement: ImprovementDirection.Down }}
  a11y={{ label: 'Clinic wait time SPC chart' }}
  ui={{ visuals: { showZones: true, rules: { enableRules: true } }, icons: { show: true }, processLineWidth: 2 as any }}
/>;
```
...
<SPCChart
  input={{ data: earlyWaitTimes, targets: Array.from({ length: earlyWaitTimes.length }, () => 20) }}
  engine={{ chartType: ChartType.XmR, metricImprovement: ImprovementDirection.Down }}
...
```ts
import { SequenceTransition } from '../../SPCChart.props';
...
- Auto baseline insertion (optional): enable the UI pre‑processor via `engine.autoRecalc` to insert a baseline after a sustained favourable shift (XmR only). Combine with `deltaSigma` to avoid over‑partitioning on negligible changes.
...
// Example: enable automatic baseline insertion (UI pre‑processor, XmR only)
<SPCChart
  input={{ data: series }}
  engine={{
    chartType: ChartType.XmR,
    ...
    autoRecalc: {
      enabled: true,
      shiftLength: 6,   // optional (default 6)
      deltaSigma: 0.5,  // optional (default 0.5)
      minGap: 12        // optional points since previous baseline
    }
  }}
  enableRules
  showIcons
...
### Automatic recalculation (UI pre‑processor)

Status: Implemented for XmR as an optional UI pre‑processor. Opt‑in via `engine.autoRecalc` to insert a synthetic baseline after a sustained favourable shift. This runs before the v2 engine build and does not change engine settings or logic; it preprocesses the input rows.
...
Key checks (current):
- XmR only; ignored for other chart types.
- Uses `shiftLength` from `engine.autoRecalc` (default 6) for sustained run length.
- Requires a favourable direction shift based on `metricImprovement` (if `Neither`, absolute change is used).
- Sigma proxy derived from average moving range (MRbar × 2.66/3) over numeric, non‑ghost points to scale the mean delta.
- Ignores ghosts and non‑numeric values when forming windows and MR.
- Optional `minGap` enforces a minimum number of points since the last baseline.
- Inserts at most one baseline per render at the first qualifying index.

Parameters (UI pre‑processor):

<Table
  caption="engine.autoRecalc"
  captionSize="s"
  head={[ { text: 'Field' }, { text: 'Type' }, { text: 'Default' }, { text: 'Notes' } ]}
  rows={[
...
Status: planned (not implemented in current v2 engine). Until available, consider surfacing warnings and manually adding baselines when a sustained shift is confirmed.
