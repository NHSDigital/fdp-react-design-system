...
} from "./examples/SPCRuleClash.examples";
...
_Polarity: lower is better (Down)._ 
...
**Commentary**: This example shows three phases: (1) a single high outlier, (2) a sustained elevation above the mean that reaches a Concern shift (because lower is better in this series), and (3) a later strictly downward trend which is favourable here. With v2 conflict pruning, the most recent sustained rule (the downward trend) governs the end‑state classification on overlapping rows; earlier shift backfill and the outlier mark are cleared where they conflict, while the isolated outlier remains only on its own point if it lies outside the trend window.
