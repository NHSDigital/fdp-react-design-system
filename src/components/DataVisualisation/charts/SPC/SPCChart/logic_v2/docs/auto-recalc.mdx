...
import SPCChartAutoRecalcMultiExample from '../doc-examples/SPCChartAutoRecalcMultiExample';
...
<Table
  ...
  rows={[
    ...
    [ { code: 'maxInsertions', codeLanguage: 'typescript' }, { code: 'number', codeLanguage: 'typescript' }, { code: '0', codeLanguage: 'typescript' }, { text: 'Optional cap for multiple automatic baseline insertions per render. Default preserves single-insert behaviour.' } ],
  ]}
  firstCellIsHeader
/>
...
### Multiple insertions (cap with maxInsertions)
...
- Multiple insertions are supported via `maxInsertions`; the default value preserves single-insert behaviour. Each insertion iteration enforces at least a 1-point progress to avoid re-selecting the same index and respects `minGap` when provided.
