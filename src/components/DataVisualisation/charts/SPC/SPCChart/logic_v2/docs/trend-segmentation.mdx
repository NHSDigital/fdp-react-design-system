...
- “Low is good” dataset fully aligns using CrossingAfterUnfavourable (see test link below).
- “High is good” dataset has no full‑match configuration found yet in a sweep across strategies with/without `preferTrendWhenConflict` and `trendDominatesHighlightedWindow`; tests report partial matches only.
