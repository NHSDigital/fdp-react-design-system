...
import { ChartType as V1ChartType, ImprovementDirection as V1ImprovementDirection } from "../../types";
...
const meta: Meta = {
  title: "Data Visualisation/SPC/v2 Logic/Retro overlay (examples)",
  parameters: {
    docs: {
      description: {
        component:
...
function makeSeries(): { x: number; y: number }[] {
  const series: number[] = [
    ...
    10.8, 10.9, 10.7, 10.85, 10.95, 10.9, 10.8, 10.92, 10.88, 10.91, 10.89, 10.93,
...
export const ShiftBackfillExample: Story = {
  ...
  render: ({ minimumPoints, direction, enableShift, showChart }) => {
    ...
    const rows = useMemo(() => {
      const input = series.map((d) => ({ x: d.x, value: d.y, ghost: false, baseline: false, target: null }));
      return buildSpcV26a({
    ...
    const overlay = useMemo(() => computeRetroShiftOverlay(rows, direction, { enableShift }), [rows, direction, enableShift]);
    ...
    const table = (
      <Table
        ...
        data={rows.map((r, i) => ({
          i,
          value: r.value,
          eligible: typeof r.mean === "number" && Number.isFinite(r.mean) ? "Yes" : "No",
    ...
    return (
      <div style={{ display: "grid", gap: 16 }}>
        <div style={{ fontSize: 12, color: "#555" }}>
          This example keeps engine outputs intact and shows a separate overlay icon for pre-eligibility points that align with a later confirmed shift. Toggle minimum points and direction to explore behaviour.
        </div>
        {showChart && (
          <div>
            <SPCChart
