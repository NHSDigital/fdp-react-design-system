...
export function toV2ImprovementDir(key: "neither" | "up" | "down" | undefined): ImprovementDirection {
  switch (key) {
...
export function normaliseSeries(group: Group) {
  const data = group.data.map(({ date, value }) => ({ x: new Date(date), value }));
  return { data };
}

// Certain metrics in the dataset encode a recalculation example; expose a helper
export function deriveBaselines(group: Group, length: number): (boolean | null | undefined)[] | undefined {
  if (group.metric === "Recalculations - Recalculated" || group.metric.startsWith("Special cause crossing recalculations")) {
