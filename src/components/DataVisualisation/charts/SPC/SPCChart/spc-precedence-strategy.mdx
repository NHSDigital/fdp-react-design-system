...
### Note on SQL Parity
- If you need to study directional pruning behavior historically used in SQL pipelines, refer to the standalone `buildSpcSqlCompat` utility at the engine layer. UI-level toggles have been removed; SPCChart, SPCMetricCard, and hooks now run the native engine consistently.
