...
- Engine-level (classification): set `disableTrendSideGating` on `SPCChart` to control gating. When `true` (default), early trend points classify directionally (Concern/Improvement) even before crossing the mean. Set to `false` to enable side‑gating. Internally, this maps to `settings.trendSideGatingEnabled = !disableTrendSideGating` in the SPC engine.
...
First, the same dataset with visuals turned OFF and engine side‑gating disabled via `disableTrendSideGating` (the default). This shows how points immediately colour as Improvement/Concern once the trend rule length is reached, regardless of being on the favourable side yet.
