...
import { TrendGatingEmbeddedHighNoGate } from './TrendGatingEmbeddedHighNoGateStory';
import Mermaid from '../../../../_internal/Mermaid';
import { ED4hTrendGatingEmbedded } from './ED4hTrendGatingEmbeddedStory';
import { ED4hTrendGatingEmbeddedNoGate } from './ED4hTrendGatingEmbeddedNoGateStory';
...
### Toggling this behaviour in SPCChart

- Engine-level (classification): set `enableTrendSideGating` on `SPCChart` to control gating. When `false` (default), early trend points classify directionally (Concern/Improvement) even before crossing the mean. Set to `true` to enable side‑gating. Internally, this maps to `settings.trendSideGatingEnabled = enableTrendSideGating` in the SPC engine.
- Visual-only (styling/copy): `enableNeutralNoJudgement={false}` hides the purple neutral styling, and `showTrendGatingExplanation={false}` hides the tooltip rationale. These do not change the classification unless side-gating is also disabled.

Note: `disableTrendSideGating` has been deprecated and replaced with the positive `enableTrendSideGating` (default OFF). A temporary compatibility alias exists, but new code should use `enableTrendSideGating`.

### Illustration (Dataset Story)

First, the same dataset with visuals turned OFF and engine side‑gating disabled by default (`enableTrendSideGating` omitted). This shows how points immediately colour as Improvement/Concern once the trend rule length is reached, regardless of being on the favourable side yet.

<TrendGatingEmbeddedHighNoGate />

Below, one can see the same dataset with the trend side-gating on: the mid‑series increasing points (e.g. 17 & 18 January) are *still below the mean* so they are purple although the trend rule has fired. Once later points rise above the mean, Improvement (blue) icons appear.

<TrendGatingEmbeddedHigh />

### Further Examples (ED 4h Compliance)

Below are two examples using an ED 4-hour compliance series to show the practical difference between disabling and enabling trend side-gating:

1. ED 4h compliance — gating OFF (directional classification occurs immediately once the trend rule triggers, regardless of side):

<ED4hTrendGatingEmbeddedNoGate />

2. ED 4h compliance — gating ON (early trend points remain neutral until the series reaches the favourable side of the mean):
