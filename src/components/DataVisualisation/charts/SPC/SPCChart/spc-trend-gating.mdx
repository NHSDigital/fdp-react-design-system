...
- Engine-level (classification): set `disableTrendSideGating` on `SPCChart` to make early trend points classify directionally (Concern/Improvement) even before crossing the mean. Internally, this maps to `settings.trendSideGatingEnabled = false` in the SPC engine.
...
First, the same dataset with visuals turned OFF and engine side‑gating disabled via `disableTrendSideGating`. This shows how points immediately colour as Improvement/Concern once the trend rule length is reached, regardless of being on the favourable side yet.
