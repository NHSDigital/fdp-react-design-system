...
import { ImprovementDirection, VariationIcon, PrecedenceStrategy, ChartType } from "./logic/spcConstants";
...
const toData = (series: { x: string; y: number }[]) =>
	series.map((p) => ({ x: new Date(p.x), value: p.y }));
