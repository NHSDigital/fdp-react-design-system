...
export const metricSeries: MetricSeries[] = Array.from(
	groupedMap.entries()
).map(([metric, points]) => ({ metric, points }));

// A lookup for quick access in tests
export const metricLookup: Record<string, { x: Date; y: number }[]> =
	metricSeries.reduce(
		(acc, s) => {
			acc[s.metric] = s.points;
			return acc;
		},
