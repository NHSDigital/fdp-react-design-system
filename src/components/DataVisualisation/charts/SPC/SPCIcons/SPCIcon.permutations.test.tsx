...
import {
	...
	SpcLetterGlyph,
} from "../SPCChart/SPCChart.constants";
...
const improving: Expectation[] = [
	{
		icon: VariationIcon.Improvement,
		improvement: ImprovementDirection.Up,
		expectedPolarityMode: "H",
		expectedDirectionMode: "H",
	},
	{
		icon: VariationIcon.Improvement,
		improvement: ImprovementDirection.Down,
		expectedPolarityMode: "L",
		expectedDirectionMode: "L",
	},
	{
		icon: VariationIcon.Improvement,
		improvement: ImprovementDirection.Neither,
		expectedPolarityMode: "",
		expectedDirectionMode: "H",
	},
];
...
const variants: Array<SpcEmbeddedIconVariant> = [
		...
		describe(`${variant} variant`, () => {
			[...improving, ...deteriorating].forEach((exp) => {
				it(`${exp.icon} / ${exp.improvement} polarity-mode`, () => {
					const { container } = render(
						<SPCVariationIcon
							data={{
								variationIcon: exp.icon,
								improvementDirection: exp.improvement,
							}}
							variant={variant}
							runLength={
			...
			neutralIcons.forEach(() => {
				(["higher", "lower"] as const).forEach((dirKey) => {
					const trend =
						dirKey === "higher" ? Direction.Higher : Direction.Lower;
				...
				const { container } = render(
					<SPCVariationIcon
						data={{
							...
							improvementDirection: ImprovementDirection.Neither,
						}}
						variant={variant}
						letterOverride={SpcLetterGlyph.H}
						runLength={
							variant === SpcEmbeddedIconVariant.TriangleWithRun ? 3 : undefined
						}
					/>
				);
				const letters = extractLetters(container);
