	...
	argTypes: {
		variant: {
			...
			options: [
				...
				SpcEmbeddedIconVariant.Triangle,
				SpcEmbeddedIconVariant.TriangleWithRun,
			],
		},
		...
		trend: {
			control: { type: "radio" },
			options: [undefined, Direction.Higher, Direction.Lower],
		},
	},
	render: ({
		variant,
		...
		<div style={{ textAlign: "center" }}>
			...
			>
				Toggle variant, icon, improvement direction or trend to preview all icon
				states.
			</div>
		</div>
	),
		...
		<Grid style={{ marginTop: 64 }}>
			<Row>
				<Column width={GridWidth.Full}>
					<Heading
						level={2}
					>{`Variant: ${variant}${gradientWash ? " (wash)" : ""}`}</Heading>
				</Column>
			</Row>
			{rows.map((group, rowIdx) => (
				<Row key={rowIdx} style={{ gap: 24, marginBottom: 32 }}>
					{group.map(({ id, label }) => {
						return (
							<Column key={id} width={GridWidth.OneHalf}>
								<div
									style={{
										...
										<Row style={{ rowGap: 16, columnGap: 16 }}>
											{(() => {
												...
												return cols.map((col) => (
													<Column
														key={`${id}-${col.key}`}
														width={GridWidth.OneHalf}
													>
														<Row style={{ gap: 12 }}>
															{col.payloads.map((pl, idx) => (
																<Column key={idx} width={GridWidth.Full}>
																	<div style={cellStyle}>
																		<SPCVariationIcon
																			data={pl}
																			size={72}
...
export const VariationIconMatrixClassic = () => (
	<>
		<VariationMatrix variant={SpcEmbeddedIconVariant.Classic} />
		<VariationMatrix variant={SpcEmbeddedIconVariant.Classic} gradientWash />
	</>
);

export const VariationIconMatrixTriangle = () => (
	<>
		<VariationMatrix variant={SpcEmbeddedIconVariant.Triangle} />
		<VariationMatrix variant={SpcEmbeddedIconVariant.Triangle} gradientWash />
	</>
);
