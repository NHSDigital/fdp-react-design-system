...
describe('SPCSpark', () => {
  ...
  it('renders svg with role img', () => {
    render(<SPCSpark data={data} variationState={VariationState.CommonCause} />);
    const svg = screen.getByRole('img', { name: /sparkline/i });
    expect(svg).toBeInTheDocument();
  });

  it('renders mean line when showMean', () => {
    render(<SPCSpark data={data} variationState={VariationState.CommonCause} showMean />);
  ...
  it('applies gradient wash when enabled', () => {
    render(<SPCSpark data={data} variationState={VariationState.SpecialCauseImproving} gradientWash />);
  ...
  it('renders shaded limit band when showLimitBand', () => {
    const dataVals = Array.from({length:30}).map((_,i)=>({ value: 50 + Math.sin(i/2)*5 }));
    render(<SPCSpark data={dataVals} variationState={VariationState.CommonCause} showLimits showLimitBand />);
    ...
    const band = rects.find(r => Number(r.getAttribute('fill-opacity')) === 0.08 || r.getAttribute('fillopacity') === '0.08');
  ...
  it('thins points when maxPoints specified and uses solid fill without stroke', () => {
    const longData = Array.from({length: 120}).map((_,i)=>({ value: Math.sin(i/5)*10 + 100 }));
    render(<SPCSpark data={longData} variationState={VariationState.CommonCause} maxPoints={40} thinningStrategy="stride" />);
  ...
  it('colours points individually when colorPointsBySignal', () => {
    ...
    const { container } = render(<SPCSpark data={data} autoClassify showLimits colorPointsBySignal />);
