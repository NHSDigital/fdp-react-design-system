...
export const ShiftExample: Story = {
  ...
  render: () => {
    ...
    const [baselines, setBaselines] = useState<Array<boolean | undefined>>(()=> Array(data.length).fill(undefined));
    ...
    return (
      <ChartContainer title="Baseline suggestion (shift)" description="Upward shift after point 14 – heuristic should propose new baseline around that point" source="Synthetic">
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <SPCChart
            ...
            narrationContext={{ measureName: 'Synthetic metric', timeframe: '30 months' }}
            gradientSequences
          />
          <SuggestionInspector data={data} baselines={baselines} onApply={applyBaseline} onReset={resetBaselines} />
        </div>
      </ChartContainer>
    );
  }
...
const annotationColor = '#005eb8'; // NHS blue
...
const SuggestionInspector: React.FC<InspectorProps> = ({ data, baselines, onApply, onReset }) => {
  const engine = buildSpc({ chartType: 'XmR', metricImprovement: ImprovementDirection.Up, data: data.map(d=> ({ x: d.x, value: d.y, baseline: undefined })), settings: { baselineSuggest: true } });
  ...
  return (
    <div style={{ marginTop: 8, border:'1px solid #d8dde0', borderRadius:4, padding:12 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <button type="button" onClick={onReset} style={{ fontSize:12, padding:'4px 8px' }}>Reset baselines</button>
      </div>
      {suggestions.length === 0 && <p style={{ margin:0 }}>No (remaining) suggestions.</p>}
      {suggestions.length > 0 && (
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
          <thead>
            <tr style={{ textAlign:'left' }}>
              <th style={{ borderBottom:'1px solid #ccc', padding:'4px 4px' }}>Index</th>
              <th style={{ borderBottom:'1px solid #ccc', padding:'4px 4px' }}>Reason</th>
              <th style={{ borderBottom:'1px solid #ccc', padding:'4px 4px' }}>Score</th>
              <th style={{ borderBottom:'1px solid #ccc', padding:'4px 4px' }}>ΔMean</th>
              <th style={{ borderBottom:'1px solid #ccc', padding:'4px 4px' }}>Window</th>
              <th style={{ borderBottom:'1px solid #ccc', padding:'4px 4px' }}>Action</th>
            </tr>
          </thead>
          <tbody>
            {suggestions.map(s => (
              <tr key={s.index}>
                ...
                <td style={{ padding:'4px 4px' }}><code>{s.reason}</code></td>
                <td style={{ padding:'4px 4px' }}>{s.score}</td>
                <td style={{ padding:'4px 4px' }}>{s.deltaMean.toFixed(2)}<br/><small>{s.oldMean.toFixed(2)} → {s.newMean.toFixed(2)}</small></td>
                <td style={{ padding:'4px 4px' }}>{s.window[0] + 1}–{s.window[1] + 1}</td>
                <td style={{ padding:'4px 4px' }}>
                  <button type="button" onClick={()=> onApply(s.index)} style={{ fontSize:12, padding:'4px 8px' }}>Apply</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <p style={{ fontSize:11, marginTop:8, lineHeight:1.4 }}>
        Applying adds a baseline flag at the suggested point and re-renders. Suggestions recompute and any at applied indices are removed. Reset clears all applied baselines.
      </p>
    </div>
  );
};
