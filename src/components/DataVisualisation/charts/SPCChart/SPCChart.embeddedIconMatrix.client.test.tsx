...
const buildImprovement = () => { const d = makeStable(); for (let i = d.length - 6; i < d.length; i++) d[i].y += 8; return d; };
const buildConcern = () => { const d = makeStable(); d[d.length - 1].y -= 10; return d; };
const buildSuppressed = () => { const d = makeStable(); d[d.length - 1].y += 40; return d; };
// Downward favourable (low) isolated single 3σ point for metricImprovement Down that should suppress to no judgement (purple)
const buildDownwardSuppressed = () => { const d = makeStable(); d[d.length - 1].y -= 40; return d; };

// Helper to extract stroke/fill colour from embedded icon container
const getIconColours = (container: HTMLElement) => {
  const ring = container.querySelector('circle[stroke]') as SVGCircleElement | null;
  const path = container.querySelector('path[fill]') as SVGPathElement | null;
  return { stroke: ring?.getAttribute('stroke') || null, fill: path?.getAttribute('fill') || null, text: container.textContent || '' };
};

describe('SPCChart embedded icon matrix', () => {
  it('renders all expected variation states with correct colours/letters', () => {
    const { container, rerender } = render(<SPCChart data={buildCommon()} metricImprovement={ImprovementDirection.Neither} showEmbeddedIcon showIcons={false} />);
    const assertCommon = () => {
      const icon = container.querySelector('.fdp-spc-chart__embedded-icon[data-variation="neither"]') as HTMLElement | null;
      ...
      if (stroke && stroke !== 'none') {
        ...
        const anyGrey = icon!.querySelector('[stroke="#A6A6A6"], [fill="#A6A6A6"]');
    ...
    // Improvement (blue H)
    rerender(<SPCChart data={buildImprovement()} metricImprovement={ImprovementDirection.Up} showEmbeddedIcon showIcons={false} />);
    const imp = container.querySelector('.fdp-spc-chart__embedded-icon[data-variation="improvement"]') as HTMLElement | null;
    expect(imp).toBeTruthy();
    const impColours = getIconColours(imp!);
    if (impColours.stroke && impColours.stroke !== 'none') {
      expect(impColours.stroke).toBe('#00B0F0');
    } else {
      const anyBlue = imp!.querySelector('[stroke="#00B0F0"], [fill="#00B0F0"]');
      expect(anyBlue).toBeTruthy();
    }
    expect(impColours.text).toContain('H');

    // Concern (orange L)
    rerender(<SPCChart data={buildConcern()} metricImprovement={ImprovementDirection.Up} showEmbeddedIcon showIcons={false} />);
    const con = container.querySelector('.fdp-spc-chart__embedded-icon[data-variation="concern"]') as HTMLElement | null;
    ...
    if (conColours.stroke && conColours.stroke !== 'none') {
      ...
      const anyOrange = con!.querySelector('[stroke="#E46C0A"], [fill="#E46C0A"]');
      expect(anyOrange).toBeTruthy();
    }
    expect(conColours.text).toContain('L');

    // Suppressed favourable (purple, variation = none => no H/L letter)
    rerender(<SPCChart data={buildSuppressed()} metricImprovement={ImprovementDirection.Up} showEmbeddedIcon showIcons={false} />);
    const sup = container.querySelector('.fdp-spc-chart__embedded-icon[data-variation="none"]') as HTMLElement | null;
  ...
  expect(sup!.getAttribute('data-trend')).toBe('higher');
    ...
    expect([supColours.stroke, supColours.fill].filter(Boolean)).toContain('#490092');
  ...
  rerender(<SPCChart data={buildDownwardSuppressed()} metricImprovement={ImprovementDirection.Down} showEmbeddedIcon showIcons={false} />);
  const downSup = container.querySelector('.fdp-spc-chart__embedded-icon[data-variation="none"]') as HTMLElement | null;
  expect(downSup).toBeTruthy();
  // Check mapped judgement & polarity attributes reflect downward metric context
  expect(downSup!.getAttribute('data-variation-judgement')).toBe('no_judgement');
  ...
  expect([downColours.stroke, downColours.fill].filter(Boolean)).toContain('#490092');
