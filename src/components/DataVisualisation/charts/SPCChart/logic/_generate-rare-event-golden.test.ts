...
describe('SPC rare-event golden parity (T & G)', () => {
  ...
  it('T chart matches expected transformed mean, MR-based limits, and signals', () => {
    const { rows, warnings } = spcModule.buildSpc({ chartType:'T', metricImprovement:ImprovementDirection.Up, data: tData, settings:{ enableFourOfFiveRule:true } });
    const last = rows[rows.length-1];
    expect(Math.abs((last.mean ?? 0) - 5.402500624503446)).toBeLessThan(tol);
    expect(Math.abs((last.mrMean ?? 0) - 0.27682757240148353)).toBeLessThan(tol);
    expect(Math.abs((last.upperProcessLimit ?? 0) - 21.145227234488576)).toBeLessThan(tol);
    expect(Math.abs((last.lowerProcessLimit ?? 0) - 0.5843168368945615)).toBeLessThan(tol);
    expect(Math.abs((last.upperTwoSigma ?? 0) - 14.17404506313916)).toBeLessThan(tol);
  ...
  it('G chart matches expected probability-based limits and emits expected warnings', () => {
    const { rows, warnings } = spcModule.buildSpc({ chartType:'G', metricImprovement:ImprovementDirection.Up, data: gData, settings:{ enableFourOfFiveRule:true } });
    const last = rows[rows.length-1];
    expect(Math.abs((last.mean ?? 0) - 7.126223234002321)).toBeLessThan(tol);
    expect(last.mrMean).toBeNull(); // G has no MR concept
    expect(Math.abs((last.upperProcessLimit ?? 0) - 76.46674882554701)).toBeLessThan(tol);
    expect(Math.abs((last.lowerProcessLimit ?? 0) - 0)).toBeLessThan(tol);
    expect(Math.abs((last.upperTwoSigma ?? 0) - 43.35277428850645)).toBeLessThan(tol);
    expect(Math.abs((last.lowerTwoSigma ?? 0) - 0)).toBeLessThan(tol);
    expect(Math.abs((last.upperOneSigma ?? 0) - 20.583515431024434)).toBeLessThan(tol);
    expect(Math.abs((last.lowerOneSigma ?? 0) - 1.0253069072922854)).toBeLessThan(tol);
