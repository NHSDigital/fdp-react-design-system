...
describe('Header (SSR)', () => {
  ...
  it('renders basic header (server variant) with classes', () => {
    const { html } = renderSSR(<HeaderServer service={{ text: 'NHS App', href: '/app' }} /> as any);
    expect(html).toContain('nhsuk-header');
    expect(html).toContain('NHS App');
  });
  ...
  it('applies white variant classes (server variant)', () => {
    const { html } = renderSSR(<HeaderServer variant="white" service={{ text: 'Service', href: '/svc' }} /> as any);
    expect(html).toContain('nhsuk-header--white');
  });
  ...
  it('combines logo + service link when hrefs match (server variant)', () => {
    const { container } = renderSSR(<HeaderServer logo={{ href: '/same' }} service={{ text: 'Portal', href: '/same' }} /> as any);
    const links = container.querySelectorAll('a[href="/same"]');
  ...
  it('server variant with overflow renders all items in dropdown only', () => {
    ...
    const { container, html } = renderSSR(<HeaderServer maxVisibleItems={3} service={{ text: 'Svc', href: '/' }} navigation={{ items: navItems }} /> as any);
    // No primary inline nav items (list present but should be empty)
    const primaryItems = container.querySelectorAll('.nhsuk-header__navigation-list > li');
    ...
    const dropdown = container.querySelector('.nhsuk-header__dropdown-menu[data-ssr-overflow="true"]');
