...
describe('Debug Hint Issues', () => {
  ...
  const { container } = renderSSR(<Hint>{''}</Hint>);
  ...
  console.log('Looking for .nhsuk-hint with smart querySelector:', container.querySelector('.nhsuk-hint'));
  ...
  const { container, getByRole } = renderSSR(
      <div>
  ...
  const { container } = renderSSR(<Hint id="screen-reader-hint">Screen reader accessible hint</Hint>);
    
    console.log('Container HTML:', container.outerHTML);
  console.log('Looking for #screen-reader-hint with smart querySelector:', container.querySelector('#screen-reader-hint'));
