...
export interface NavigationSplitViewProps<ID = string, T extends NavigationSplitItem<ID> = NavigationSplitItem<ID>> {
   ...
   */
  autoContentHeader?: boolean | { mobile?: boolean; tablet?: boolean; desktop?: boolean };
  /**
  ...
  renderContentHeader?: (args: {
    item: T; // selected item (always defined when called)
    detailActive: boolean; // true when in mobile detail slide state
    context: 'mobile' | 'tablet' | 'desktop'; // coarse breakpoint context
    backLink?: React.ReactNode; // standard BackLink node (only when mobile detail + mobile header enabled)
    defaultHeading: React.ReactNode; // <h{level}>{item.label}</h{level}>
  }) => React.ReactNode;
}
