...
export const ResponsiveDataGrid: React.FC<ResponsiveDataGridProps> = ({
	...
	const [cardNavState, setCardNavState] = useState<CardNavigationState>({
		...
		focusArea:
			hideTabsIfSingle && tabPanels.length === 1 ? "sort-controls" : "tabs",
		focusedTabIndex: 0,
		focusedCardIndex: 0,
		selectedCardIndex: -1,
		isGridActive: false,
	...
	const gridToIndex = useCallback(
		(row: number, col: number, columns: number) => {
	...
	useEffect(() => {
		if (
			...
			cardNavState.focusedCardElementIndex >= 0 &&
			cardNavState.cardElements.length > 0
		) {
			// Use a small timeout to ensure the DOM has updated
			setTimeout(() => {
				const currentElement =
					cardNavState.cardElements[cardNavState.focusedCardElementIndex];
				if (currentElement) {
	...
	const sortCardData = useCallback(
		(data: any[], sortConfig: SortConfig | null) => {
	...
	const sortCardDataCombined = useCallback(
		(data: any[]) => {
			const currentPanel = tabPanels[state.selectedIndex];
	...
	const focusCardElement = useCallback(
		(cardIndex: number, elementIndex: number) => {
		...
		(controlIndex: number) => {
			if (controlIndex === 0) {
				...
				if (formControl) {
					...
					const elementType = isSelect
						? "dropdown"
						: isButton
							? "button"
							: "control";
					const keyboardHint = isSelect
						? ". Use Space key to open dropdown"
						: "";
	...
	const handleCardKeyDown = useCallback(
		(event: React.KeyboardEvent, cardIndex: number) => {
			const { key } = event;
			const currentPanel = tabPanels[state.selectedIndex];
					...
					case " ":
						if (currentPanel?.data[cardIndex]) {
							...
							setCardNavState((prev) => ({
								...prev,
								selectedCardIndex:
									prev.selectedCardIndex === cardIndex ? -1 : cardIndex,
							}));
	...
	const handleTabKeyDownWithCards = useCallback(
	...
	const handleSortControlKeyDown = useCallback(
		(event: React.KeyboardEvent, controlIndex: number) => {
				...
				switch (key) {
					...
					case "ArrowRight":
						...
						const nextControlIndex =
							cardNavState.focusedSortControlIndex < controlCount
								? cardNavState.focusedSortControlIndex + 1
			...
			>
							...
							{tabPanels.map((panel, index) => {
								...
								return (
									<button
										...
										onKeyDown={(event) =>
											handleTabKeyDownWithCards(event, index)
										}
										disabled={isDisabled}
										className={[
											...
											.join(" ")}
									>
										<span className="aria-tabs-datagrid__tab-label">
											{panel.label}
										</span>
				...
				>
					{sortCardDataCombined(currentPanel?.data || []).map((row, index) => {
						...
						const isFocused =
							cardNavState.focusedCardIndex === index &&
							cardNavState.focusArea === "cards";
						const isInCardNavigation =
							cardNavState.focusedCardIndex === index &&
							cardNavState.focusArea === "card" &&
							cardNavState.isCardNavigationActive;
						// Ensure first card is focusable if no card is currently focused
						const isFirstCardFocusable =
							index === 0 && cardNavState.focusArea !== "cards";
									...
									<div
										...
										className={[
											...
											isFocused
												...
												: "",
											isInCardNavigation
												? "aria-tabs-datagrid-adaptive__card-wrapper--card-navigation"
												: "",
										]
										.filter(Boolean)
										.join(" ")}
										...
										aria-description={
											isInCardNavigation
												? `Card navigation active. ${cardNavState.cardElements.length} interactive elements available.`
												: undefined
										}
										...
										onClick={() => {
											setCardNavState((prev) => ({
												...prev,
												selectedCardIndex:
													prev.selectedCardIndex === index ? -1 : index,
											}));
										...
										onFocus={() => {
											...
											setCardNavState((prev) => {
												...
												if (
													prev.focusedCardIndex !== index ||
													prev.focusArea !== "cards"
												) {
													return {
														...prev,
