 ...
 */
export const AriaDataGrid = React.forwardRef<
	HTMLTableElement,
	AriaDataGridProps
>(
	(
		...
		const [navigationMode, setNavigationMode] =
			useState<NavigationMode>("browse");
			...
			(event: React.KeyboardEvent) => {
				...
				switch (key) {
					...
					case "ArrowLeft":
						...
						if (
							navigationMode === "navigate" ||
							(navigationMode === "browse" && focusArea === "headers")
						) {
				...
				<thead className="nhsuk-table__head" role="rowgroup">
					<tr ref={headerRowRef} role="row">
						{columns.map((column, colIndex) => {
							...
							const isFocused =
								focusArea === "headers" && focusedColumnIndex === colIndex;
							...
							return (
								<th
									...
									onClick={() => {
										// Sync mouse interaction with keyboard focus model
										setFocusArea("headers");
										setFocusedColumnIndex(colIndex);
										setNavigationMode("navigate");
										focusHeader(colIndex);
										// Preserve existing click-to-sort behaviour
				...
				<tbody ref={tbodyRef} className="nhsuk-table__body" role="rowgroup">
					{sortedData.map((row, rowIndex) => {
						...
						return (
							...
							>
								{columns.map((column, colIndex) => {
									const value = column.tableRenderer
										? column.tableRenderer(row)
										: column.render
											? column.render(row)
											: row[column.key];
									const isCellFocused =
										...
										<td
											...
											tabIndex={isCellFocused ? 0 : -1}
												onClick={() => {
													...
													// Preserve existing click-to-select behaviour
