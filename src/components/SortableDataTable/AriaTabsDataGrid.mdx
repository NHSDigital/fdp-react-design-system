...
<Meta title="NHS/Data/AriaTabsDataGrid/Documentation" />

# AriaTabsDataGrid Component

The `AriaTabsDataGrid` is a fully accessible data grid component that combines tabbed navigation with sortable data tables. Originally designed for NHS healthcare workflows, it has been generified to support any domain while maintaining specialised healthcare functionality.
...
const healthcareTabs = createHealthcareTabsConfig(patientData);
```
...
The component uses NHS design tokens and can be customized via CSS:
