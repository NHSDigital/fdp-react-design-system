...
### Column Rendering Precedence & Per-Column Boolean Rendering
...
| Per Column | `column.customRenderer(value, row)` | Custom rendering for that column (any type). Overrides global booleanRenderer when used |

If a column supplies `booleanRenderer`, it overrides the global one for that column only.
...
|-------|-----|----------|
| Global | `dataConfig.booleanRenderer` | Consistent boolean display across all columns/tabs |
| Per Column | `column.booleanRenderer(value, row)` | Special casing a single boolean column (e.g. icons, badges) |
...
const tabPanels = createGenericTabsConfig(products, [
  {
    ...
    columns: [
      ...
      { 
        key: 'inStock', 
        label: 'Availability',
        booleanRenderer: (val: boolean) => val ? <span className="status status--ok">Available</span> : <span className="status status--no">Out</span>
      },
      { 
        key: 'price', 
        label: 'Price', 
