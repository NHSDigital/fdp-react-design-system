 ...
 */
export interface TransactionalPageTemplateProps
 ...
 * </TransactionalPageTemplate>
 */
export const TransactionalPageTemplate: React.FC<
...
> = ({
	...
	mainWrapperSize = "small", // Default to small size for transactional forms
	skipLinkProps,
	headerProps,
	footerProps,
	backLinkProps,
	...
	// Prepare simplified header configuration for transactional use
	const headerConfig = {
		service: {
			text: serviceName,
			href: serviceHref,
	...
	return (
		<>
			...
			<WidthContainer className={className} {...containerProps}>
				{/* Back link - appears before main content */}
				{backLinkProps && <BackLink {...backLinkProps} />}
				<MainWrapper size={mainWrapperSize}>
					<Row>
						<Column width={columnWidth}>
							{ title && (
								<Heading style={{ marginTop: "3rem" }} size="l" {...pageHeadingProps}>
									{title}
								</Heading>
							)}
							{children}
