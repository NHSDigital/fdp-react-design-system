  ...
  return new Promise<void>((resolve, reject) => {
    const attempt = () => {
      ...
      try {
        ...
        req.setTimeout(1200, () => { req.destroy(); setTimeout(attempt, intervalMs); });
...
function ensureLockDir() { if (!fs.existsSync(LOCK_DIR)) fs.mkdirSync(LOCK_DIR); }

interface OwnedProcess extends StartedProcess { owned: boolean }

async function startWithLock(options: { name: string; url: string; healthPath?: string; scriptDecider: () => { script: string; timeout: number }; }): Promise<OwnedProcess> {
  ensureLockDir();
  ...
  async function quickHttpCheck(): Promise<boolean> {
    try { await waitForHttp(`${url}${healthPath}`, 500, 100); return true; } catch { return false; }
  }

  function tryKillPort(p: number) {
    if (!process.env.DS_KILL_STALE) return false;
    try {
      const out = execSync(`lsof -iTCP:${p} -sTCP:LISTEN -Pn || true`, { stdio: 'pipe' }).toString();
      ...
      for (const line of lines) {
        ...
        const pid = parts[1];
        if (pid) {
          try { process.kill(Number(pid), 'SIGKILL'); console.warn(`[server-utils] killed stale process ${pid} on port ${p}`); } catch {}
        }
      }
      return true;
    } catch { return false; }
  ...
  try { await waitForHttp(`${url}${healthPath}`, 700, 120); return { proc: undefined as any, url, stop: () => {}, owned: false }; } catch {/* not up */}
  ...
  if (fs.existsSync(lockFile)) {
    ...
    try {
      ...
      console.warn(`[server-utils] stale lock detected for ${name}, removing and respawning...`);
  ...
  try {
    const portOut = execSync(`lsof -iTCP:${port} -sTCP:LISTEN -Pn || true`, { stdio: 'pipe' }).toString();
    if (portOut.trim()) {
      const responsive = await quickHttpCheck();
      if (!responsive) {
        console.warn(`[server-utils] port ${port} occupied by unresponsive process. Set DS_KILL_STALE=1 to auto-kill.`);
  ...
  console.log(`[server-utils] starting ${name} via npm run ${script} (timeout ${timeout}ms)`);
  const proc = spawn('npm', ['run', script], { cwd: ROOT, stdio: 'pipe', env: { ...process.env } });
  let sawAddrInUse = false;
  proc.on('exit', (code, signal) => {
    if (code !== 0) {
      console.error(`[server-utils] Child process ${name} exited early code=${code} signal=${signal}`);
    }
  });
  const debug = !!process.env.DS_RUNTIME_DEBUG;
  if (debug && proc.stdout && proc.stderr) {
    proc.stdout.on('data', d => process.stdout.write(`[${name}-stdout] ${d}`));
  ...
  try {
    ...
    return { proc, url, stop: () => { try { proc.kill(); } catch {} fs.existsSync(lockFile) && fs.unlinkSync(lockFile); }, owned: true };
  } catch (e) {
    // If address in use we adopt existing server and proceed (not owned)
    if (sawAddrInUse) {
      console.warn(`[server-utils] adopting pre-existing ${name} server (EADDRINUSE)`);
      try { fs.existsSync(lockFile) && fs.unlinkSync(lockFile); } catch {}
...
export async function startExpress(): Promise<StartedProcess> {
  ...
  healthPath: '/healthz',
    scriptDecider: () => ({ script: fast ? 'examples:express:serve' : 'examples:express:start', timeout: Number(process.env.DS_SERVER_START_TIMEOUT || 6000) })
  });
  return started;
}
...
export async function startNext(): Promise<StartedProcess> {
  ...
  healthPath: '/healthz',
    scriptDecider: () => {
      ...
      const script = fast ? 'examples:next:dev:fast' : (nextDir ? 'examples:next:start:fast' : 'examples:next:start');
      const timeout = Number(process.env.DS_SERVER_START_TIMEOUT || (fast ? 15000 : 20000));
...
export async function waitForBehavioursGlobal(page: any, timeoutMs = 8000) {
  try {
    await page.waitForFunction(() => !!(window as any).__nhsInitAllBehaviours, { timeout: timeoutMs });
  } catch (e) {
    // Emit some diagnostics to help debug why missing
    const html = await page.content();
    // eslint-disable-next-line no-console
    console.error('[diagnostic] behaviours global missing after wait. First 500 chars of HTML:', html.slice(0, 500));
    throw e;
  }
}
